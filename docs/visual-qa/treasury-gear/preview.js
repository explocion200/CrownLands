...
function slotMarkup(slot){return `<button type="button" class="tg-slot${slot.equipped?"":" is-empty"}" data-slot="${slot.slot}" aria-pressed="${currentModel().selectedSlot===slot.slot}" aria-label="${esc(slot.label+(slot.equipped?`, ${slot.equippedDefinition.gearName}, Level ${slot.equipped.level}`:", empty"))}"><span class="tg-slot-art">${slot.equipped?art(slot.equippedDefinition.art):icon("plus")}</span><span>${slot.label}<small>${slot.equipped?"Lv. "+slot.equipped.level:"Empty"}</small></span>${slot.isUpgradeReady?'<b class="tg-ready" title="Matching upgrade material available">!</b>':""}</button>`;}
function bagMarkup(group){const item=group.representative,def=group.definition;return `<button type="button" class="tg-item${group.isEquipped?" equipped":""}${group.isCompatible?" compatible":""}" data-item="${item.instanceId}" aria-pressed="${group.isSelected}" aria-label="${esc(`${def.gearName}, Level ${item.level}, ${group.count} owned${group.isEquipped?", equipped":""}${group.isNew?", new":""}`)}" title="${esc(def.gearName)}"><span class="tg-item-mark">${group.isEquipped?icon("check"):group.isNew?'<b>New</b>':""}</span>${art(def.art)}<span class="tg-item-level">Lv. ${item.level}</span>${group.count>1?`<span class="tg-item-count">×${group.count}</span>`:""}<span class="tg-item-name">${esc(shortName(def.gearName))}</span>${group.isUpgradeReady&&!group.isEquipped?'<b class="tg-ready" title="Matching upgrade material available">!</b>':""}</button>`;}
...
function showConfirmation(){const vm=currentModel();if(!vm.canMerge||sample==="gold")return;const item=vm.selected,req=vm.requirement;confirmation=true;returnFocus="[data-action=upgrade]";
  document.body.insertAdjacentHTML("beforeend",`<dialog class="tg-confirm" aria-labelledby="tgConfirmTitle"><div>${icon("needle")}<p class="tg-eyebrow">Treasury workshop</p><h2 id="tgConfirmTitle">Upgrade ${esc(shortName(vm.definition.gearName))}?</h2><div class="tg-combine">${art(vm.definition.art)}<span>Level ${item.level}<br>+ 1 matching copy</span><b aria-hidden="true">→</b><span>Level ${item.level+1}<strong>+${percent(vm.nextBonus)}%</strong></span></div><p>Combine this Level ${item.level} item with ${req.duplicates} unequipped matching Level ${item.level} copy. Both inputs are consumed to create one new Level ${item.level+1} item.</p><dl><div><dt>Gold cost</dt><dd>${number(vm.upgradeGold)}</dd></div><div><dt>Raw production</dt><dd>${req.baseGoldHours}h</dd></div></dl><p class="tg-warning">This two-to-one upgrade cannot be undone.</p><footer><button type="button" data-action="cancel">Cancel</button><button class="tg-primary" type="button" data-action="confirm">Confirm Upgrade</button></footer></div></dialog>`);
...
window.addEventListener("message",event=>{if(event.origin!==location.origin||event.source!==parent||event.data?.type!=="treasury-review")return;const next=event.data;if(next.layout)layout=next.layout;if(next.sample)sample=next.sample;if(next.reset){selected={ready:"draft-1",missing:"draft-9",max:"draft-15",empty:"empty",gold:"draft-1"}[sample]||"draft-1";filter="all";}render();});
