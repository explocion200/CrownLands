...
const settings={layout:"draft",viewport:["landscape","small"].includes(query.get("viewport"))?query.get("viewport"):"desktop",sample:"ready",portrait:["animated","still","original"].includes(query.get("portrait"))?query.get("portrait"):"animated"};
...
function update(reset=false){document.querySelectorAll("button[data-layout],button[data-viewport]").forEach(button=>{const key=Object.keys(button.dataset)[0];button.setAttribute("aria-pressed",String(settings[key]===button.dataset[key]));});document.getElementById("portrait").value=settings.portrait;document.getElementById("portrait").disabled=settings.layout!=="draft";preview.contentWindow?.postMessage({type:"treasury-review",...settings,reset},location.origin);size();}
...
document.getElementById("portrait").addEventListener("change",event=>{settings.portrait=event.target.value;update();});
document.getElementById("reset").addEventListener("click",()=>update(true));
window.addEventListener("message",event=>{if(event.origin!==location.origin||event.source!==preview.contentWindow||event.data?.type!=="treasury-status")return;document.getElementById("reviewStatus").textContent=event.data.message;});
