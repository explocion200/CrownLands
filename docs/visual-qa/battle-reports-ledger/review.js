...
function reset() {
  frame.contentWindow?.postMessage({type:"reports-review", sample:examples.value}, location.origin);
...
window.addEventListener("message", event => {
  if (event.origin === location.origin && event.source === frame.contentWindow && event.data?.type === "reports-review-status") document.getElementById("reviewStatus").textContent = event.data.message;
});
