...
function row(report) {
  ...
  return `<article class="report-row ${report.tone}${isNew ? " new" : ""}" data-report-id="${report.id}" aria-label="${escapeHtml(report.label+': '+report.city)}">
    <div class="report-result">${art(report.type,"report-art")}<div class="result-copy"><div class="result-line"><strong class="outcome">${escapeHtml(report.label)}</strong>${isNew ? '<span class="new-tag">New</span>' : ""}</div><span class="time">${escapeHtml(report.age)}${report.expiry ? `<span class="expiry${report.urgent ? " urgent" : ""}">Expires in ${escapeHtml(report.expiry)}</span>` : ""}</span></div></div>
