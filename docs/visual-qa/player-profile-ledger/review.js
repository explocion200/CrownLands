...
function reset() {
  ...
  frame.contentWindow?.postMessage({ type: "profile-review", sample: document.getElementById("sample").value }, location.origin);
}
...
window.addEventListener("message", event => {
  if (event.origin === location.origin && event.source === frame.contentWindow && event.data?.type === "profile-review-status") document.getElementById("reviewStatus").textContent = event.data.message;
});
