...
const examples = document.getElementById("sample");
...
function syncUrl() {
  history.replaceState(null, "", `?viewport=${viewport}&sample=${examples.value}`);
}
function reset() {
  frame.contentWindow?.postMessage({type:"settings-review", sample:examples.value}, location.origin);
...
document.querySelectorAll("[data-viewport]").forEach(button => button.addEventListener("click", () => {
  viewport = button.dataset.viewport; resize(); syncUrl();
}));
examples.addEventListener("change", reset);
document.getElementById("reset").addEventListener("click", reset);
frame.addEventListener("load", reset);
window.addEventListener("message", event => {
  if (event.origin === location.origin && event.source === frame.contentWindow && event.data?.type === "settings-review-status") document.getElementById("reviewStatus").textContent = event.data.message;
});
