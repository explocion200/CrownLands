"use strict";
/* Design preview only: no game entry, account, network writes or saved preferences. */
const chapters=window.CrownlandsHelpDraft,$=s=>document.querySelector(s),dialog=$('#helpDialog'),article=$('#article');
const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
const icon=src=>src.includes('#')?`<svg viewBox="0 0 64 64" aria-hidden="true"><use href="${esc(src)}"></use></svg>`:`<img src="${esc(src)}" alt="">`;
let selected='first-steps',query='',tips=false;const scrolls=new Map(),expanded=new Set();
function notify(message){$('#liveStatus').textContent=message;parent.postMessage({type:'help-status',message,chapter:selected},location.origin);}
...
$('#tips').addEventListener('click',()=>{tips=!tips;$('#tips').setAttribute('aria-pressed',String(tips));$('#tipsNote').textContent=tips?'First steps guidance enabled':'Guidance on the map and in Profile';notify(tips?'Tips enabled in this preview only.':'Tips disabled in this preview only.');});
function close(){dialog.close();$('#dismissed').hidden=false;notify('Help preview closed.');}
$('#closeHelp').addEventListener('click',close);$('#backToGame').addEventListener('click',close);dialog.addEventListener('cancel',e=>{e.preventDefault();close();});$('#reopen').addEventListener('click',()=>{$('#dismissed').hidden=true;dialog.showModal();});
window.addEventListener('message',e=>{if(e.origin===location.origin&&e.source===parent&&e.data?.type==='help-review'){expanded.clear();scrolls.clear();chooseChapter(e.data.chapter,{reset:true});if(!dialog.open){$('#dismissed').hidden=true;dialog.showModal();}}});
const start=new URLSearchParams(location.search).get('chapter');if(chapters.some(c=>c.id===start))selected=start;render();dialog.showModal();document.documentElement.dataset.helpReady='true';
