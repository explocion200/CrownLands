...
function reset() {
  frame.contentWindow?.postMessage({type:"battle-detail-review", sample:examples.value}, location.origin);
...
window.addEventListener("message", event => {
  if (event.origin === location.origin && event.source === frame.contentWindow && event.data?.type === "battle-detail-status") document.getElementById("reviewStatus").textContent = event.data.message;
});
