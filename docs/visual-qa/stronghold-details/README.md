# Gold Stronghold details draft

Status: **DRAFT — awaiting design approval**. The user requested this first Gold Stronghold detail draft as the next part of the Leaderboards + Map Selector + objective-details review batch, continuing `codex/leaderboard-ledger-draft`. The Leaderboards and Map Selector designs are approved. This batch is not integrated or released.

Review `/docs/visual-qa/stronghold-details/index.html?viewport=desktop&sample=owned`. Desktop is 1440 × 900; mobile landscape 844 × 390 and small landscape 568 × 320. No portrait game layout. The window uses the City Details envelope, up to 1040 × 790 pixels, and fits within the landscape viewport with eight-pixel margins.

## Presentation

The crimson/gilt frame follows the approved atlas royal maps. A gold-coin seal identifies Aurum Keep's specialization, and the current packaged fortress illustration occupies the left column. Troop count follows the image; owner/clan profile links, defense level and visibility/garrison information remain below. The right column leads with the bonus and defense estimate, followed by wall condition, detailed effects and reinforcement controls. Both columns scroll independently on short screens. The management action remains in the fixed footer; Legacy has a shorter footer to give rankings more space.
...
This pass covers **Gold Stronghold only**. Training, Movement, Defense and the Crown Citadel detail/Reign Ledger designs will follow user approval of this arrangement. No attack, scout, Rally, city upgrade, wall upgrade or Inner Castle button has been invented for this information dialog.
...
Current Core objective identity/art is taken from `functions/core-expansion-world-layout.json`, the packaged catalog, and the existing Strongholds ledger fixture. **Do not copy old 20%/25% layout metadata**: current runtime and confirmed bonuses are 8% direct / 4% clan sharing, subject to Citadel precedence. The main owned example assumes only the Gold Stronghold objective; it is not a replacement total-benefit calculator.

Fixture arithmetic follows `functions/economy-config.json`: Level 50 base walls are 1,456,669; 12% Stoneworks plus 4% Gear gives 1,689,736 full wall power. A 3,250,000 owner garrison at 1.30 base defense gives an intact local estimate of 5,914,736. The damaged example uses 60% integrity (1,013,841 wall power), giving 5,238,841 estimated defense. The full-breach repair window is 30 minutes. Support is separately attributed, consistent with the current locally estimated display; these values are not an authoritative battle forecast.
