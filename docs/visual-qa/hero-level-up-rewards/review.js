...
function replay() { frame.contentWindow?.postMessage({type:"hero-reward-review",sample:sample.value},location.origin); sync(); }
...
sample.addEventListener("change",replay);
document.getElementById("reset").addEventListener("click",replay);
window.addEventListener("resize",resize);
window.addEventListener("message",event => {
  if(event.origin!==location.origin || event.source!==frame.contentWindow)return;
  if(event.data?.type==="hero-reward-ready")replay();
  if(event.data?.type==="hero-reward-status")document.getElementById("reviewStatus").textContent=event.data.message;
});
