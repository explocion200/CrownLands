...
function resize() {
  const [width, height] = sizes[viewport], scale = Math.min(1, Math.max(0.1, (document.documentElement.clientWidth - 24) / width));
...
function reset() {
  frame.contentWindow?.postMessage({ type: "incoming-review", sample: examples.value }, location.origin);
...
window.addEventListener("message", event => {
  if (event.origin === location.origin && event.source === frame.contentWindow && event.data?.type === "incoming-status") document.getElementById("reviewStatus").textContent = event.data.message;
});
