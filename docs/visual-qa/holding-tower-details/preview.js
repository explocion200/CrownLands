...
function wallsMarkup(f) {
  ...
  const veilReason = f.veil ? "Wait for the current Veil to end." : !f.veilUses ? "Daily uses spent. Resets at 00:00 UTC." : f.treasury < f.veilCost ? `Need ${number(f.veilCost-f.treasury)} more gold.` : "";
  ...
  const veil = `<article class="defense-pane veil-work"><header class="defense-pane-heading">${icon("veil")}<div><h2>Veil of Silence</h2><p>Protection from scouting</p></div><span class="state-chip ${f.veil ? "good" : f.member && !f.veilUses ? "warning" : ""}">${f.veil ? "Active" : f.member ? f.veilUses ? "Ready" : "Spent" : "Inactive"}</span></header><div class="defense-scroll" tabindex="0" aria-label="Veil status and service"><div class="veil-purpose"><h3>Conceal the garrison</h3><p>Blocks scouting of the tower for 10 minutes. Clan ownership and wall condition remain public.</p></div>${f.veil ? '<div class="veil-countdown"><small>Active Veil ends in</small><strong>7 min</strong></div>' : ""}<div class="veil-facts"><div><small>Duration</small><strong>10 <span>min</span></strong></div>${f.member ? `<div><small>Uses left today</small><strong>${f.veilUses} <span>/ 3</span></strong><div class="veil-uses" aria-hidden="true">${[0,1,2].map(i=>`<span class="${i<f.veilUses?"available":""}"></span>`).join("")}</div></div>` : ""}</div>${f.member ? `<p class="service-caption reset-note">Daily allowance resets at 00:00 UTC.</p><section class="veil-activation">${cost("Cost per use",f.veilCost)}${f.manager ? action("veil",f.veil ? "Veil Active" : "Activate Veil","primary-button",Boolean(veilReason)) : '<p class="service-caption">A leader or officer can activate this service.</p>'}${veilReason ? `<p class="service-blocker ${f.veil ? "calm" : ""}">${veilReason}</p>` : ""}</section>` : '<p class="service-privacy">Daily usage and activation are private to the controlling clan.</p>'}<p class="veil-payment">Paid from Clan Treasury.<br>Your Bag items are not used.</p><details class="service-details"><summary>How the service is priced</summary><p>Each use costs the equivalent city wall upgrade price at the tower's current wall level.</p></details></div></article>`;
