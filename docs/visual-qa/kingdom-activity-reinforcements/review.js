...
function reset() {
  frame.contentWindow?.postMessage({ type: "reinforcements-review", sample: examples.value }, location.origin);
...
window.addEventListener("message", event => {
  if (event.origin === location.origin && event.source === frame.contentWindow && event.data?.type === "reinforcements-status") document.getElementById("reviewStatus").textContent = event.data.message;
});
