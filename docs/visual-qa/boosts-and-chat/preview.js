...
(() => {
  ...
  let sample="standard",view="boosts",channel="global",target="en",selected="drums",miniOpen=true,epoch=0,started=Date.now(),messages=[],nextId=0,sendUntil=0;
  ...
  function renderEffects(){
    const current=active();$("effectCount").textContent=current.length+" active";
    if(current.length&&!current.some(e=>e.id===selected))selected=current[0].id;
    $("effectRack").innerHTML=current.map(e=>'<button type="button" class="effect-token '+(remaining(e)<300?'expiring':'')+'" data-effect="'+e.id+'" aria-label="'+esc(e.name)+', '+time(remaining(e))+' remaining"><img src="'+art(e)+'" alt=""><time data-timer="'+e.id+'">'+time(remaining(e))+'</time></button>').join("");
    if(!current.length)$("effectRack").innerHTML='<button class="effect-token empty-token" type="button" data-open-boosts>No active effects</button>';
    $("boostList").hidden=!current.length;$("effectDetail").className="";
    if(!current.length){$("effectDetail").innerHTML='<div class="empty-effects"><strong>No active effects</strong>Your kingdom has no timed boosts or protection.<br>Activated items will appear here.</div>';$("effectDetail").style.gridColumn="1 / -1";return;}
    $("effectDetail").style.gridColumn="";
    $("boostList").innerHTML=current.map(e=>'<button type="button" class="boost-option '+(remaining(e)<300?'expiring':'')+'" data-effect="'+e.id+'" aria-pressed="'+(selected===e.id)+'"><img src="'+art(e)+'" alt=""><span><strong>'+e.name+'</strong><span class="subline"><i class="status-dot"></i><span>'+(remaining(e)<300?'Ending soon':'Active')+'</span><time data-timer="'+e.id+'">'+time(remaining(e))+'</time></span></span></button>').join("");
    renderDetail(current.find(e=>e.id===selected));
  }
  function renderDetail(e){
    const soon=remaining(e)<300;$("effectDetail").classList.toggle("effect-expiry",soon);
    $("effectDetail").innerHTML='<div class="effect-hero"><img src="'+art(e)+'" alt="'+e.name+'"><div><span class="detail-category">'+e.category.toUpperCase()+'</span><h2>'+e.name+'</h2><p class="effect-benefit">'+e.benefit+'</p></div></div><div class="detail-timer"><span>'+(soon?'ENDING SOON':'TIME REMAINING')+'</span><time data-timer="'+e.id+'">'+time(remaining(e))+'</time></div><div class="time-track" aria-hidden="true"><span style="width:'+Math.min(100,remaining(e)/e.total*100)+'%"></span></div><dl class="effect-facts"><div><dt>Applies to</dt><dd>'+e.scope+'</dd></div><div><dt>Effect</dt><dd>'+e.description+'</dd></div></dl><p class="effect-note">'+e.note+'</p>';
  }
  function setView(next){
    view=["boosts","map","chat"].includes(next)?next:"map";
    for(const dialog of [$("boostDialog"),$("chatDialog")])if(dialog.open)dialog.close();
    if(view==="boosts"){$("boostDialog").showModal();renderEffects();}
    if(view==="chat"){renderMessages();$("chatDialog").showModal();$("chatMessageList").scrollTop=$("chatMessageList").scrollHeight;}
    report(view==="map"?"Arrow toggles mini chat · tap an effect for details":view==="boosts"?"Choose an effect to inspect its scope and timer":"Translate works on one message at a time");
  }
  ...
  function seed(){
    messages=[
      ...
      makeMessage("Marshal Alden","en",1)
    ];
  ...
  function renderMessages(){
    ...
    $("quickChatMessages").innerHTML=messages.slice(-2).map(m=>'<div class="mini-row"><div class="mini-line"><strong>'+esc(m.person)+'</strong><span class="mini-text" dir="auto">'+esc(shown(m))+'</span></div>'+toolsFor(m,true)+'</div>').join("");
  ...
  function reset(options={}){
    ...
    seed();$("chatMessageInput").value="";$("chatStatus").hidden=true;$("newMessages").hidden=true;updateComposer();selectChannel("global",false);setMini(true);renderEffects();setView(options.view||"boosts");
  }
  ...
  document.addEventListener("click",e=>{
    ...
    const effect=e.target.closest("[data-effect]");if(effect){selected=effect.dataset.effect;if(view!=="boosts")setView("boosts");else renderEffects();return;}
    if(e.target.closest("[data-open-boosts]"))setView("boosts");
  ...
  $("openLedger").addEventListener("click",()=>setView("chat"));$("openChat").addEventListener("click",()=>setView("chat"));$("openBoosts").addEventListener("click",()=>setView("boosts"));$("allEffects").addEventListener("click",()=>setView("boosts"));
  for(const id of ["closeBoosts","backToMap","chatMinimizeBtn","chatCloseBtn"])$(id).addEventListener("click",()=>{if(id==="chatMinimizeBtn")setMini(true);setView("map");});
  for(const dialog of [$("boostDialog"),$("chatDialog")])dialog.addEventListener("cancel",e=>{e.preventDefault();setView("map");});
