...
const sizes={desktop:[1440,900],landscape:[844,390],small:[568,320]};let viewport=Object.hasOwn(sizes,query.get('viewport'))?query.get('viewport'):'desktop';
...
function reset(){frame.contentWindow?.postMessage({type:'shop-review',sample:document.getElementById('sample').value,artwork:document.getElementById('artwork').value},location.origin);}
...
document.getElementById('artwork').addEventListener('change',()=>frame.contentWindow?.postMessage({type:'shop-artwork',artwork:document.getElementById('artwork').value},location.origin));
window.addEventListener('message',e=>{if(e.origin!==location.origin||e.source!==frame.contentWindow||e.data?.type!=='shop-status')return;document.getElementById('reviewStatus').textContent=e.data.message;});
