...
for(const id of ["sample","level","layout"])if([...document.getElementById(id).options].some(o=>o.value===query.get(id)))document.getElementById(id).value=query.get(id);
...
function reset(){const url=new URL(location.href);url.searchParams.set("viewport",viewport);for(const id of ["sample","level","layout"])url.searchParams.set(id,document.getElementById(id).value);history.replaceState(null,"",url);frame.contentWindow?.postMessage({type:"castle-layout-review",sample:document.getElementById("sample").value,layout:document.getElementById("layout").value,level:Number(document.getElementById("level").value)},location.origin);}
document.querySelectorAll("[data-viewport]").forEach(b=>b.addEventListener("click",()=>{viewport=b.dataset.viewport;resize();reset();}));
for(const id of ["sample","level","layout"])document.getElementById(id).addEventListener("change",reset);
