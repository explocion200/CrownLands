...
function reset() {
  ...
  frame.contentWindow?.postMessage({ type: "skills-review", sample: document.getElementById("sample").value }, location.origin);
}
...
window.addEventListener("message", event => {
  if (event.origin === location.origin && event.source === frame.contentWindow && event.data?.type === "skills-review-status") document.getElementById("reviewStatus").textContent = event.data.message;
});
window.addEventListener("resize", resize); resize();

const galleryNames = {swordmastery:"Swordmastery",marchOrders:"March Orders",fieldMedics:"Field Medics",shieldwallDiscipline:"Shieldwall Discipline",stoneworks:"Stoneworks",taxStewardship:"Tax Stewardship",royalGranaries:"Royal Granaries",guildCharters:"Guild Charters"};
document.getElementById("iconGallery").innerHTML = Object.entries(galleryNames).map(([id,label]) => `<figure><img src="icons/${id}.svg" alt=""><figcaption>${label}</figcaption></figure>`).join("");
