...
const shortName=value=>value.replace("Defensive Commander's ","");
const art=(src,alt="")=>`<img src="${esc(src)}" alt="${esc(alt)}" draggable="false">`;
function officerMarkup(){return `<figure class="tg-officer has-character">${art("docs/visual-qa/gatehouse-gear/art/defensive-commander-still-v1.webp","Defensive Commander in worn steel and a moss-green cloak, resting a hand on a fortress shield")}<figcaption>Warden of the walls</figcaption></figure>`;}
const icons={
...
function announce(message){parent.postMessage({type:"gatehouse-status",message},location.origin);}
...
function render(){
  ...
  const vm=currentModel();document.body.className=layout==="draft"?"gatehouse-draft":"gatehouse-current";
  if(layout==="current"){
...
function showConfirmation(){const vm=currentModel();if(!vm.canMerge||sample==="gold")return;const item=vm.selected,req=vm.requirement;confirmation=true;returnFocus="[data-action=upgrade]";
  document.body.insertAdjacentHTML("beforeend",`<dialog class="tg-confirm" aria-labelledby="tgConfirmTitle"><div>${icon("forge")}<p class="tg-eyebrow">Gatehouse armoury</p><h2 id="tgConfirmTitle">Upgrade ${esc(shortName(vm.definition.gearName))}?</h2><div class="tg-combine"><span class="tg-confirm-art" data-rarity="${esc(vm.definition.rarity)}">${art(vm.definition.art)}</span><span>Level ${item.level}<br>+ 1 matching copy</span><b aria-hidden="true">→</b><span>Level ${item.level+1}<strong>+${percent(vm.nextBonus)}%</strong></span></div><p>Combine this Level ${item.level} item with ${req.duplicates} unequipped matching Level ${item.level} copy. Both inputs are consumed to create one new Level ${item.level+1} item.</p><dl><div><dt>Gold cost</dt><dd>${number(vm.upgradeGold)}</dd></div><div><dt>Raw production</dt><dd>${req.baseGoldHours}h</dd></div></dl><p class="tg-warning">This two-to-one upgrade cannot be undone.</p><footer><button type="button" data-action="cancel">Cancel</button><button class="tg-primary" type="button" data-action="confirm">Confirm Upgrade</button></footer></div></dialog>`);
...
window.addEventListener("message",event=>{if(event.origin!==location.origin||event.source!==parent||event.data?.type!=="gatehouse-review")return;const next=event.data;if(next.layout)layout=next.layout;if(next.sample)sample=next.sample;if(next.reset){selected={ready:"draft-1",missing:"draft-9",max:"draft-15",weapon:"draft-13",empty:"empty",gold:"draft-1"}[sample]||"draft-1";filter="all";}render();});
fetch("docs/visual-qa/gatehouse-gear/snapshot.json").then(r=>{if(!r.ok)throw Error("Could not load the Gatehouse sample.");return r.json();}).then(async data=>{snapshot=data;for(const href of [...data.stylesheets,"docs/visual-qa/gatehouse-gear/draft.css"]){const link=document.createElement("link");link.rel="stylesheet";link.href=href;if(!href.startsWith("https:")&&!href.endsWith("/draft.css"))link.dataset.legacy="true";document.head.append(link);}await Promise.all([...document.querySelectorAll("link[rel=stylesheet]")].map(link=>link.sheet?Promise.resolve():new Promise((resolve,reject)=>{link.onload=resolve;link.onerror=()=>reject(Error("Could not load preview styles."));})));render();document.documentElement.dataset.ready="true";}).catch(error=>{document.body.textContent=error.message;});
