...
Approved September 22, 2026. `clan-shop-ui.js/css` now provides the actual Tower Store and Shop building view. It uses live server prices/stock, real personal Gold and Bag contents, existing idempotent purchase and construction actions, and the current membership and construction permissions. Tower Info and the building selector retain access to the other three buildings. The selection and scroll positions survive live refreshes. Successful and failed purchases show inline status; no cost or allowance is calculated from draft sample data. Publication remains pending.
