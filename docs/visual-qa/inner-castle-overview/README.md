...
- Mobile landscape moves the existing Back to City Details button into the top bar beside Close. The footer notice moves into the detail pane, freeing the footer's height for the Royal Bailey. The full 4:3 image stays visible even on short landscape screens; all six building signs retain their fixed positions and at least 44-pixel-high touch targets. The detail pane scrolls if needed.
- The mobile detail pane stacks the building name, a centered preview image up to 160 pixels wide, the existing information and new-gear indicator, then Manage Gear where available. The pane scrolls to reach lower content and returns to its top when a different building is selected. Desktop presentation is unchanged by this refinement.
- The same Back button and notice move back to the desktop footer when the viewport changes, preserving selection and avoiding duplicate controls. This draft does not change the game's orientation guard or create a portrait game layout.
...
The mobile building-detail refinement additionally checks all six buildings at four landscape sizes: centered 158–160-pixel artwork, title/image/information/action order, returning to the top on a different selection, and scrolling to click Manage Gear. The earlier desktop layout and enlarged Royal Bailey scene remain unchanged.
