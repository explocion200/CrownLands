...
For the latest Gatehouse candidate, open <http://127.0.0.1:61703/docs/visual-qa/inner-castle-overview/index.html?viewport=landscape&building=gatehouse&review=gatehouse-v1>. The optional `building` parameter selects an existing building in either comparison; unrecognized values fall back to Great Hall.
...
- The draft uses new Royal Bailey, Great Hall, Treasury, Barracks, Alehouse and Gatehouse illustrations following the user's map reference: dark ink outlines, muted olive and ochre, pale stone and burgundy pennants. Gatehouse is the latest interior candidate. Royal Stables remains original for individual review later. Current layout retains the original hub and all original interior images for comparison.
