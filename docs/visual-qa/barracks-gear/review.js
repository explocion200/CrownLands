...
window.addEventListener("message",event=>{if(event.origin!==location.origin||event.source!==preview.contentWindow||event.data?.type!=="barracks-status")return;document.getElementById("reviewStatus").textContent=event.data.message;});
