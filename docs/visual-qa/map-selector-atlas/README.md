...
The ornamental revision adds original SVG frames in `frames.js`: green masonry with battlement corners and a tower shield; riveted brass Camp fittings with a tent seal; crimson royal frames with gold scroll corners. Each Stronghold crest reuses its own fortress landmark art from the current catalog: Greybanner Hold's training compound, Aurum Keep's gold fortress, Swiftgate's mounted stronghold and Ironwatch's defensive fortress. Only Crown Citadel retains the gold crown crest. Lower corners stay clear of captions. Overlays are decorative and ignore pointer input; the established color trims remain visible when fine ornament is hidden at Whole realm overview scale. Tile positions, artwork registration, caption space and window dimensions are unchanged.
