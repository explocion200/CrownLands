...
The ornamental revision adds original SVG frames in `frames.js`: green masonry with battlement corners and a tower shield; riveted brass Camp fittings with a tent seal; crimson royal frames with gold scroll corners and a crown crest, with extra gold accents for the Citadel. Lower corners stay clear of captions. Overlays are decorative and ignore pointer input; the established color trims remain visible when fine ornament is hidden at Whole realm overview scale. Tile positions, artwork registration, caption space and window dimensions are unchanged.
