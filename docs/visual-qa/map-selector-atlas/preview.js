...
function tile(r){const p=position(r),kind=feature(r),current=r.id===currentId,home=r.id===homeId,count=ownCount(r),royal=catalog.mapPresentation.redTrimRegionIds.includes(r.id);return `<button class="map-tile ${royal?"royal":""} ${r.objectivePurpose==="holding_tower"?"tower":""} ${r.campCount>0?"camp":""} ${current?"current":""} ${home?"home":""}" data-region="${r.id}" data-grid-x="${r.gridX}" data-grid-y="${r.gridY}" style="left:${p.x-tileW/2}px;top:${p.y-tileH/2}px" tabindex="${current?0:-1}" aria-label="Open ${esc(describe(r))}">${royal?atlasFeatureFrame("royal",r.objectivePurpose==="crown_citadel",esc(r.artPresentation?.landmarks?.find(landmark=>landmark.kind==="stronghold")?.asset||"")):""}${r.objectivePurpose==="holding_tower"?atlasFeatureFrame("tower"):""}${r.campCount>0?atlasFeatureFrame("camp"):""}<span class="map-thumb">${thumb(r)}</span><span class="map-name">${esc(r.name)}</span><span class="map-meta"><span class="map-owned">Your cities: <strong>${count===null?"Syncing":count}</strong></span><span class="feature-label">${kind.split(" · ").filter(Boolean).map(label=>`<span class="feature-badge ${label==="Tower"?"tower":label==="Camp"?"camp":"royal"}">${label}</span>`).join("")}</span></span>${current||home?`<span class="map-flags">${current?'<span class="map-flag"><span class="full-label">Current map</span><span class="short-label">Here</span></span>':""}${home?'<span class="map-flag home-flag"><span class="full-label">Home map</span><span class="short-label">Home</span></span>':""}</span>`:""}${current?'<span class="map-state-marker" aria-hidden="true"></span>':""}</button>`;}
function render(){
...
viewport.addEventListener("focusin",event=>{const el=event.target.closest("[data-region]");if(!el)return;$("mapInfo").textContent=describe(byId.get(el.dataset.region));if(pointers.size)return;const r=el.getBoundingClientRect(),v=viewport.getBoundingClientRect();if(r.left<v.left||r.right>v.right||r.top<v.top||r.bottom>v.bottom)centerRegion(el.dataset.region);});
