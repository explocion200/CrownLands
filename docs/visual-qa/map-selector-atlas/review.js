...
function reset(){frame.contentWindow?.postMessage({type:"atlas-review",sample:examples.value},location.origin);syncUrl();}
...
window.addEventListener("message",event=>{if(event.origin===location.origin&&event.source===frame.contentWindow&&event.data?.type==="atlas-status")document.getElementById("reviewStatus").textContent=event.data.message;});resize();
