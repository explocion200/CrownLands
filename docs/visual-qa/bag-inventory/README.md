...
Status: **DRAFT — awaiting visual approval.** Created 12 September 2026 on `codex/bag-inventory-draft`. This folder is an isolated review; no live game entry point loads it. No pull request, merge, or deployment is part of this draft task.
...
- Existing item illustrations are carried forward for this UI review. No new raster artwork is included. Bag consumables and unopened boxes have parchment wells, with no invented rarity classification. The approved gray Common-equipment backgrounds in other screens are unaffected.
...
The Master Specification's Shop and Items / Item behavior section requires All, Boosts, War, Defense, Utility; identical-item quantity grouping; eight visible positions; and paging/navigation. Those rules are preserved. No specification amendment is made before the proposed design is approved.
...
Local screenshots and detailed verification output are kept in the ignored `release-artifacts/bag-inventory/` folder. Physical-device touch and runtime integration remain future checks. Release-wide emulator/CI gates have not been run for this approval-only draft; the update is not ready to merge. If approved for integration, preserve the existing action pipeline and run the repository's required release preparation before a PR is marked ready.
