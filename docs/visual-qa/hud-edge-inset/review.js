...
function sync(){
 ...
 frame.contentWindow.postMessage({type:"hud-edge-review",look,zoom:Number(zoom.value),guide:guide.checked},location.origin);
}
...
window.addEventListener("message",event=>{if(event.origin===location.origin&&event.source===frame.contentWindow&&event.data?.type==="hud-edge-status")document.getElementById("reviewStatus").textContent=event.data.message;});
