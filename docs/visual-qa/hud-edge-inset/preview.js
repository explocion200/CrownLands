...
(() => {
 ...
 const status=message=>{if(parent!==window)parent.postMessage({type:"hud-edge-status",message},location.origin);};
 function update(){if(ready)frame.contentWindow.HudEdgeDraft.apply(settings);}
 window.addEventListener("message",event=>{
  if(event.origin!==location.origin)return;
  if(event.source===parent&&event.data?.type==="hud-edge-review"){
  ...
  if(event.source===frame.contentWindow&&event.data?.type==="hud-edge-status")status(event.data.message);
 });
 frame.addEventListener("load",async()=>{
  try{
   ...
   const style=doc.createElement("link");style.rel="stylesheet";style.href="/docs/visual-qa/hud-edge-inset/draft.css";
   const styled=new Promise((resolve,reject)=>{style.onload=resolve;style.onerror=reject;});doc.head.append(style);await styled;
   const script=doc.createElement("script");script.src="/docs/visual-qa/hud-edge-inset/runtime-fixture.js";
   const loaded=new Promise((resolve,reject)=>{script.onload=resolve;script.onerror=reject;});doc.body.append(script);await loaded;
   ready=true;notice.hidden=true;update();document.documentElement.dataset.hudEdgeReady="true";
