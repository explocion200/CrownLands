...
const B=window.CrownlandsClanTowerBuildings,$=selector=>document.querySelector(selector),dialog=$("#workshopDialog");
...
function notify(message){$("#liveStatus").textContent=message;parent.postMessage({type:"workshop-status",message:message+" · Draft only"},location.origin);}
function reset(level=4,sample="ready"){
 ...
 if(current<10&&["upgrading","paused"].includes(sample)){
  ...
  model.project={building:"workshop",target:current+1,remaining,end:Date.now()+remaining,paused:sample==="paused",total:B.duration(current+1)};
  model.treasury-=B.cost(current+1);
 }
 if(sample==="other"&&current<10)model.project={building:"infirmary",target:5,remaining:5400000,end:Date.now()+5400000,paused:false,total:B.duration(5)};
 render();if(!dialog.open)dialog.showModal();$("#dismissed").hidden=true;notify("Workshop example loaded.");
}
function reason(){
 ...
 if(model.project)return model.project.building!=="workshop"?"The Infirmary is being upgraded. Only one building project can run at a time.":model.project.paused?"Construction resumes automatically once the walls are fully repaired and incoming attacks end.":"Construction is underway. The completed Workshop level remains active.";
...
function projectCard(){
 const project=model.project,active=project?.building==="workshop",paused=active&&project.paused;
 ...
 else if(project){title=active?(paused?`Level ${project.target} upgrade paused`:`Building Level ${project.target}`):"Another building is underway";badge=paused?"Paused":active?"In progress":"Infirmary";copy=reason();}
...
function startUpgrade(){
 ...
 const token=epoch,shouldFail=model.failOnce;model.failOnce=false;model.pending=true;model.failed=false;model.feedback="";renderOverview();renderFooter();notify("Starting a sample upgrade…");
 requestTimer=setTimeout(()=>{
  ...
  else{const target=model.level+1,total=B.duration(target);model.treasury-=B.cost(target);model.project={building:"workshop",target,total,remaining:total,end:Date.now()+total,paused:false};model.feedback="Sample construction started. The current benefit remains active.";}
  render();notify(model.feedback);
 },450);
}
function tick(){
 const project=model?.project;if(!project)return;
 if(!project.paused&&project.end<=Date.now()){if(project.building==="workshop")model.level=project.target;model.project=null;render();notify("Sample construction completed.");return;}
...
function close(){epoch++;clearTimeout(requestTimer);model.pending=false;dialog.close();$("#dismissed").hidden=false;notify("Workshop preview closed.");}
$("#upgrade").addEventListener("click",startUpgrade);$("#closeWorkshop").addEventListener("click",close);dialog.addEventListener("cancel",event=>{event.preventDefault();close();});$("#reopen").addEventListener("click",()=>{$("#dismissed").hidden=true;render();dialog.showModal();});
...
window.addEventListener("message",event=>{if(event.origin===location.origin&&event.source===parent&&event.data?.type==="workshop-review")reset(event.data.level,event.data.sample);});
setInterval(()=>{if(dialog.open)tick();},1000);
const query=new URLSearchParams(location.search);reset(query.has("level")?Number(query.get("level")):4,query.get("sample")||"ready");document.documentElement.dataset.workshopReady="true";
