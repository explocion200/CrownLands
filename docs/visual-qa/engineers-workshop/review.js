...
function reset(){updateUrl();frame.contentWindow.postMessage({type:"workshop-review",...settings()},location.origin);}
...
window.addEventListener("message",event=>{if(event.origin===location.origin&&event.source===frame.contentWindow&&event.data?.type==="workshop-status")document.getElementById("reviewStatus").textContent=event.data.message;});
