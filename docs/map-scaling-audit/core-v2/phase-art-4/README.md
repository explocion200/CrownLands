# Crownlands Core v2 Phase ART-4
...
ART-2 v2 and ART-3 remain the authoritative art standard. The generated-player 118-asset library is unchanged.

ART-4 is approved for a development art checkpoint. Fresh interactive runtime QA is explicitly deferred because the external Codex Browser permission layer rejected the healthy local fixture. This deferred gate is not a runtime pass and remains production-blocking. Before any Core v2 production replacement, live-world migration, activation, or season-reset use, all 25 finished Core maps—including these five ART-4 maps—must pass one consolidated interactive Core QA run.
