...
- `functions/test/emulator-first-time-onboarding.js` exercises actual Auth, Firestore rules, and Functions in the current Core expansion realm: fresh admission, one city/100 Gold/1,000 troops, matching troop statistics, progress-preserving claim retries, two-device handoff, stale join/heartbeat/leave rejection, protected session writes, concurrent login serialization, identity/ownership preservation, and notification isolation.
