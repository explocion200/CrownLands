...
- A subsequent tooling-only change applies the tested token-refresh endpoint guard to compatibility and soak runners as well. Its regression rejects Firebase auth/data and both function generations while permitting loopback assets and fonts. Recorded browser measurements retain their original harness identities.
