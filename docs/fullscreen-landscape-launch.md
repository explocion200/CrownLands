...
The game manifest requests `standalone` with `landscape` orientation. This restores a window that the game's fullscreen button and X can control: manifest-owned fullscreen cannot be exited with the DOM fullscreen API. The manifest identity, start route and scope remain `/play/`, `/play/` and `/`. Installed launches still enter fullscreen from the first entry gesture where supported. The existing standalone/iOS homepage-shortcut recovery remains intact. Fullscreen mode alone does not redirect the public homepage because a normal browser window can also be fullscreen.

The fullscreen control remains visible. DOM fullscreen shows an X labeled Exit fullscreen; ordinary play shows Enter fullscreen. An older installation still using browser-owned fullscreen shows Fullscreen help with browser-exit guidance. The browser must apply the manifest update; reopening or reinstalling from the canonical game route may be needed. Web code cannot force an older native fullscreen window back into standalone mode. Icon URLs are relative so the same files load under an itch.io upload path.
