...
September 20 follow-up: the user superseded the Boosts overview and larger mini-chat presentation with the [compact map HUD](visual-qa/compact-map-hud/README.md). The historical implementation below was deployed in PR #315; the follow-up deliberately retires its overview while preserving shared art, individual translation and combat mechanics.
...
- Full validation exposed an existing world-travel fixture collision: a hardcoded secondary city could overwrite an actor's allocated Main City. The fixture now reserves both real Main Cities and distinct travel targets. Production Main City ownership checks remain unchanged.
