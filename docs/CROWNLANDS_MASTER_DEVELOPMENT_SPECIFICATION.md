...
### Approved Active Boosts and mini Chat completion — September 19, 2026

- Active Boosts & Protection uses the approved parchment list/detail layout and shared Shield, War Drums, Royal Tax Decree and Veil illustrations. Existing effect deadlines, production values and scopes remain authoritative. Expiring effects are highlighted; empty and expired states remain understandable. Clicking a HUD effect opens its details; Active effects opens the whole list. Swift March and Recall remain per-order consumables, and deferred Clan Tower Veil mechanics are not part of this presentation.
- Mini Chat restores the previous `cl-icon-back` arrow, reversed while expanded, and the exact previous brown background: a 72%-opacity gradient from `rgba(49,33,23,.72)` to `rgba(22,16,12,.72)`, with the original subtle repeating grain. It retains a separate Open chat action, per-message translation and readable text. These controls work on desktop and mobile landscape.
