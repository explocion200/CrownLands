...
- Direct `+1`, `+5`, and `MAX` controls show their real Gold costs. The header shows projected available Gold. City names open information and locate controls navigate to the holding. Existing authoritative affordability, queue, roster completeness, sorting and five-holding pagination rules remain unchanged. The roster scrolls between the fixed summary and footer; compact screens stack the same information with at least 44px controls.
- Only this panel and the confirmed owned-city Inner Castle shortcut correction are included in this update. The standalone review prototype uses illustrative data and is excluded from production. Design confirmation does not establish deployment.
...
- This design update is limited to regular City Details. The map, HUD, Stronghold details, Citadel, Inner Castle, and other dialogs retain their existing presentation. City List follows its separately confirmed presentation below. This confirmed design does not establish that the integration has been deployed.
