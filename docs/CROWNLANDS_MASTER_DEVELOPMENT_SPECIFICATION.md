...
- Confirmed chat translation: Translate targets the device/browser preferred language, remembers the player's on/off preference on that device, and applies to incoming messages in the open channel while enabled. Originals remain available, including during loading or failure; failure offers a manual retry. Reading older messages keeps its position and shows a New messages jump instead of scrolling automatically.
