...
- Troop production uses `floor(city victory points x 10.3)`, plus Royal Granaries and objective bonuses.
...
- Each defending soldier contributes `1.30` base defense power. Shieldwall Discipline adds 2% per level up to 60%; personal and shared clan objective support add against the same base. City level never changes soldier defense. Reinforcements use their sender's live Shieldwall and objective support but do not duplicate the destination wall.
- Regular-city walls use a smooth staged curve: 200 at Level 1, 600 at Level 2, 1,456,669 at Level 50, 3,000,000 at Level 100, and 6,200,000 at Level 150. Levels 101-150 tie each gain to the Gold cost of that upgrade. Above Level 150 the curve transitions to a base troop-production replacement ratio and reaches 240 hours at Level 200, then continues with troop production. Stoneworks is the only wall-strength multiplier and adds up to 75%; objective support never changes wall power. `round(15 + city level x 0.3)` minutes is the full-breach repair window. Wall damage of at least 5% persists and adds its exact share of that window to the running deadline. Integrity and wall defense recover continuously as the timer counts down, reaching 100% at the deadline; later meaningful hits use that recovered strength, preserve elapsed progress, and add only their own damage time. Combat captures, relinquishment, inactivity release, neutralization, neutral claims, ownership changes, and capture level drops preserve the active wall integrity and deadline. If the wall absorbs all attack power, all defending soldiers and reinforcements survive unharmed, including an exact breach with no power left over. Only remaining attack power can damage the garrison; protected raids follow this rule and never persist wall damage.
- Realm information advertises `siegeCombatVersion: 1` and `defenseCombatVersion: 1`. Newly launched city and objective armies use both; hostile armies already in flight without the defense version settle with legacy city-level soldier defense and objective-strengthened walls. Every neutral reward camp starts with 20,000 troops and has no level or wall; each stationed troop and reinforcement contributes exactly `1.00` defense power with no skill or objective bonuses.
...
- Four private skill presets unlock at Hero Levels 25, 50, 75, and 100. Current Build and preset drafts share a `− | cost | +` control: live changes project immediately, additions spend the displayed weighted cost, and removals freely refund the exact removed-level cost. Reset Skills is free and preserves saved presets. Saving, renaming, and inspecting presets are free; every confirmed Apply costs one hour of base gold production, even when the allocation already matches. Stored legacy reset credits are retained but are no longer consumed.
...
- The Citadel Legion selects up to 20 random regular non-main cities in the Crown Citadel region at 9:45 AM and 6:15 PM Eastern Time, then attacks each with 100,000 NPC troops at 10:00 AM and 6:30 PM Eastern. The `America/New_York` schedule follows daylight-saving changes. Citadel attacks ignore all city-wall defense without damaging the wall. Defeating every stationed and reinforcing troop removes five city levels, with Level 5-or-lower cities returning to neutral at Level 1 with 10 troops. Peace Shields do not block the event, held defenses preserve city level, and defenders receive no XP.
...
- Daily Missions live under Daily Rewards → Quests and reset at 00:00 UTC. The server locks three achievable, capacity-scaled missions and their production-based rewards for the day, tracks progress from validated combat, camp, economy, upgrade, Stronghold, and Clan Gift events, and allows one unfinished mission reroll. Rewards are claimed manually and remain visible as completed until reset.
- Common Gear Boxes appear in the Bag and reveal exactly three server-rolled Level 1 Common pieces. Players receive boxes at weekly daily-login milestones, for completing all three daily missions, from a 1% bonus on rewarded Relic Camp payouts, or by buying one per UTC day for one raw Gold-production hour plus the capped kingdom-size premium. Gear is managed in the main city Inner Castle through the Barracks, Treasury, Royal Stables, and Gatehouse officer screens; Great Hall and Alehouse gear remain unavailable. Each upgrade combines two matching same-level items into one newly identified next-level item: both input identities are consumed, total inventory falls by one, and an equipped target transfers its slot to the result. Direct Gold charges are 0.5, 1, 2, or 4 hours of current raw regular-city gold production for Levels 1→2 through 4→5. A finished Level 5 piece therefore represents 16 Level 1 copies and 16 cumulative raw-production hours. Upgrade request IDs make retries idempotent. Equipped bonuses apply additively to base combat, economy, movement, wall, defense, recovery, scouting, or repair values. Gear inventory, box opening, purchases, equipment, and upgrades are server-authoritative and cannot be changed through normal profile saves.
...
- Global and Clan Chat use bounded realm-scoped Firestore listeners and the `sendChatMessage` callable; clients cannot write messages directly. See `docs/CHAT_ARCHITECTURE.md` for paths, authorization, retention, lifecycle, and rollout notes.
- Clan Rallies are server-authoritative 2–20-player attacks against Strongholds and the Crown Citadel. Only Clan Leaders and Officers may form them; the creator or Clan Leader may launch or cancel after every contribution is Ready, while only the creator controls the combined army's Recall Horn. If that creator leaves, is removed from, or changes clans after launch, the server recalls the entire Rally automatically. Forming targets live in clan-private Firestore documents. Participant attack packages and proportional losses settle separately, and allied survivors remain as attributed reinforcements when the Rally captures its objective.
...
- Every non-Citadel Stronghold has its own Stronghold Legacy ledger. It ranks rulers by cumulative control time for that specific Stronghold, carries prior tenures forward, and keeps the current holder's timer rising live until control changes. The Crown Citadel continues to use its separate Reign Ledger.
- The Clan War Room is the clan home for Rallies. Leaders and Officers can form them; all members can join, inspect, and withdraw before launch. A clan may maintain up to five active Rallies, each with 2–20 participants.
