...
- Four private skill presets unlock at Hero Levels 25, 50, 75, and 100. Current Build and preset drafts share a `− | cost | +` control: live changes project immediately, additions spend the displayed weighted cost, and removals freely refund the exact removed-level cost. Reset Skills is free and preserves saved presets. Saving, renaming, and inspecting presets are free; every confirmed Apply costs 1,000,000 gold, even when the allocation already matches. Stored legacy reset credits are retained but are no longer consumed.
