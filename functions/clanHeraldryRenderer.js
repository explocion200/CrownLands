(function initializeClanHeraldryRenderer(root, factory) {
  ...
  function create({ config, assets, legacyRenderer }) {
    ...
    function renderV2Markup(shield, options = {}) {
      ...
      return `<span class="clan-heraldry-v2 clan-heraldry-${variant} clan-heraldry-${escapeHtml(shield.finish)}" data-heraldry-version="2" data-heraldry-variant="${variant}" style="${style}"><svg viewBox="0 0 100 106" role="img" aria-label="${label}" focusable="false"><path class="clan-heraldry-backplate" d="${shape.path}"/><g class="clan-heraldry-boundary" clip-path="url(#clan-v2-clip-${escapeHtml(shield.shape)})"><rect class="clan-heraldry-field" width="100" height="106"/>${divisionMarkup(shield)}<g class="clan-heraldry-charges">${chargesMarkup(shield, variant)}</g>${texture}${scratches}<path class="clan-heraldry-border" d="${shape.path}"/>${inner}${rivets}</g></svg></span>`;
    }
