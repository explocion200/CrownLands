...
routeWorkerScope.onmessage = event => {
  const message = event.data || {};
  if (message.type === "warmup") {
    routeWorkerScope.postMessage({ type: "warmup", ok: true });
    return;
  }
  if (message.type === "routeBatch") {
    try {
      ...
      routeWorkerScope.postMessage({ type: "routeBatch", id: message.id, ok: true, routes });
    } catch (error) {
      routeWorkerScope.postMessage({
        type: "routeBatch",
        id: message.id,
        ok: false,
  ...
  try {
    ...
    routeWorkerScope.postMessage({ type: "route", id: message.id, ok: true, route });
  } catch (error) {
    routeWorkerScope.postMessage({
      type: "route",
      id: message.id,
