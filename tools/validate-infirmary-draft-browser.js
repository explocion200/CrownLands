...
const directory=path.resolve(__dirname,"../release-artifacts/infirmary");fs.mkdirSync(directory,{recursive:true});
...
(async()=>{
 ...
 try{
  ...
  for(const[width,height]of[[1440,900],[844,390],[568,320]]){
   ...
   assert.equal(result.art,B.art("infirmary",4));assert.equal(await evaluate('$("#currentBenefit").textContent'),"+6%");assert.equal(await evaluate('$("#nextBenefit").textContent'),"+7.5%");
   await capture(width+"-overview");
   await evaluate('$("#recoveryExample").open=true;$("#recoveryExample").scrollIntoView({block:"end"})');
   assert.equal(await evaluate('$("#exampleTotal").textContent'),"36%");assert.equal(await evaluate('$("#exampleNext").textContent'),"37.5%");
   await evaluate('$("#exampleBase").value="88";$("#exampleBase").dispatchEvent(new Event("change",{bubbles:true}))');
   assert.equal(await evaluate('$("#exampleTotal").textContent'),"90%");assert.equal(await evaluate('$("#exampleNext").textContent'),"90%");
   assert(await evaluate('$("#exampleCap").textContent.includes("Only 2 of")'));assert(!(await measure()).overflow);assert((await measure()).footerVisible);
   await capture(width+"-recovery-cap");
   await evaluate('$("#recoveryExample").open=false');
   ...
   assert.deepEqual(await evaluate('({balance:model.treasury,level:model.level,target:model.project.target,current:$("#currentBenefit").textContent})'),{balance:4260000000-B.cost(5),level:4,target:5,current:"+6%"});
   ...
   for(const sample of["paused","attack","damage","other","low","member","unavailable"]){
    ...
    assert.equal(await evaluate('$("#currentBenefit").textContent'),"+6%");
    if(sample==="paused"){
   ...
   for(const level of[0,1,7,10]){
    ...
    assert.equal(await evaluate('$("#currentBenefit").textContent'),"+"+B.bonus("infirmary",level)+"%");assert.equal(await evaluate('$("#buildingArt").getAttribute("src")'),B.art("infirmary",level));
  ...
  await client.send("Page.navigate",{url:address.url+"/docs/visual-qa/infirmary/index.html?viewport=landscape&level=4"});
  await wait('document.getElementById("preview")?.contentDocument?.documentElement?.dataset.infirmaryReady==="true"');
