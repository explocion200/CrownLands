...
async function main() {
  ...
  const logging = require(path.join(lib, "gcp/cloudlogging"));
  ...
  if (await fs.stat(path.join(output, "production.json")).catch(() => null)) throw Error("Refusing to overwrite production evidence.");
  ...
  report.requested={days:selectedDays,queries:selectedQueries};
  for (const days of selectedDays) {
    ...
    for (const [kind, clause] of Object.entries({ http: 'httpRequest.latency:* AND httpRequest.requestMethod="POST"', errors: "severity>=ERROR", timings: '(jsonPayload.requestDurationMs:* OR textPayload:"crownlands_operation" OR jsonPayload.message:"crownlands_operation")' })) {
      if(!selectedQueries.includes(kind))continue;
      const filter = `resource.type="cloud_run_revision" AND ${clause} AND timestamp>="${since}" AND timestamp<="${endedAt}"`;
      ...
      do {
        ...
        for (const entry of response.entries) {
          ...
          if (kind === "http") {
            ...
            const category = ["out of memory", "memory limit", "deadline_exceeded", "resource_exhausted", "failed_precondition", "permission_denied", "unauthenticated", "unavailable", "index", "timeout", "transaction"].find(value => message.includes(value)) || "other";
            g.errorCategories[category] = (g.errorCategories[category] || 0)+1;
          }
        }
        token = response.nextPageToken;
      } while (token && pages < PAGE_LIMIT);
      window.queries[kind] = { status: failure ? "unverified" : token ? "partial" : "complete", failure, count, pages, truncated: Boolean(token), groups: Object.values(groups).map(g => ({...g,
  ...
  process.exitCode = report.windows.some(w => Object.values(w.queries).some(q => q.status !== "complete")) ? 1 : 0;
  console.log(`Recorded ${functions.length} backend revisions and current realm identity. No production data changed.`);
}
main().catch(error => { console.error(JSON.stringify({message:error.message, original:error.original?.message, status:error.status, contextStatus:error.context?.response?.statusCode})); process.exitCode = 1; });
