/* Browser integration coverage for the approved Barracks, using synthetic gear and a local API stub. */
const fs = require('node:fs'), path = require('node:path'), assert = require('node:assert/strict');
const root = path.resolve(__dirname, '..'), out = path.join(root, 'release-artifacts/barracks-gear');
...
async function main() {
  ...
  try {
    ...
    assert(await evaluate("modal.querySelector('[data-barracks-officer]').getAttribute('src')===BARRACKS_OFFICER_ART"), 'The Captain must remain static after confirmation.');
    ...
    await evaluate("state.gear=normalizeCommonGearState({});renderCommonGearBuilding('barracks')");
    ...
    await evaluate(`state.gear=normalizeCommonGearState(${JSON.stringify(gear)});selectedCommonGearInstanceId=${JSON.stringify(ready)};renderCommonGearBuilding('barracks');window.__barracksCalls=[];window.__barracksOriginalApi=getOnlineApi;getOnlineApi=()=>({unequipCommonGear:args=>new Promise((resolve,reject)=>{window.__barracksCalls.push({action:'unequip',args});window.__barracksReject=reject;}),equipCommonGear:async args=>{window.__barracksCalls.push({action:'equip',args});throw Error('Synthetic equip rejection');},upgradeCommonGear:args=>new Promise(resolve=>{window.__barracksCalls.push({action:'upgrade',args});window.__barracksResolve=resolve;})});`); await paint();
    ...
    await evaluate("modal.querySelector('[data-gear-equip]').click();window.__barracksReject(Error('Synthetic unequip rejection'))");
    await wait('!commonGearActionInFlight'); assert.equal(await evaluate('__barracksCalls.length'), 1);
    await evaluate(`selectedCommonGearInstanceId=${JSON.stringify(stored)};renderCommonGearBuilding('barracks')`); await paint();
    ...
    assert(await evaluate("selectedCommonGearInstanceId==='barracks-upgraded' && createCommonGearViewModel('barracks').selected.level===3 && Object.keys(state.gear.instances).length===15"));
    await evaluate('getOnlineApi=window.__barracksOriginalApi');
    for (const mode of ['reduced','off','full']) {
      await evaluate(`setAnimationModePreference('${mode}')`); await paint();
      assert(await evaluate("modal.querySelector('[data-barracks-officer]').getAttribute('src')===BARRACKS_OFFICER_ART && modal.querySelector('[data-barracks-officer]').getAnimations().length===0"));
    }
    await evaluate("selectedCommonGearInstanceId=Object.values(state.gear.instances).find(i=>i.slot==='weapon').instanceId;renderCommonGearBuilding('barracks')");
    assert(await evaluate("modal.querySelector('.tg-details').textContent.includes('Attack strength for all attacks')"));
    ...
    for (const building of ['gatehouse','royal-stables']) {
      await evaluate(`renderCommonGearBuilding(${JSON.stringify(building)})`); await paint();
      assert(await evaluate(building === "gatehouse" ? "!!modal.querySelector('.tg-shell [data-gatehouse-officer]')" : "!!modal.querySelector('.common-gear-screen')&&!modal.querySelector('.tg-shell')"));
