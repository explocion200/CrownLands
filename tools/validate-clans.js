...
requires(rules, /match \/giftActivity\/\{resetId\}[\s\S]*?allow get: if clanMember\(clanId\)[\s\S]*?resetId == currentRealmStorageId\(\)[\s\S]*?allow list: if false[\s\S]*?allow create, update, delete: if false;/, "Clan gift donor history is not restricted to current clan members and the active shard.");
...
requires(firebaseClient, /function subscribeClanApplications[\s\S]*?where\("resetGeneration",\s*"==",\s*RESET_GENERATION\)[\s\S]*?where\("worldId",\s*"==",\s*ONLINE_WORLD_ID\)[\s\S]*?\.\.\.getRealmShardQueryConstraints\(where\)[\s\S]*?where\("status",\s*"==",\s*"pending"\)[\s\S]*?onSnapshot/, "Clan managers do not have a legacy-compatible shard application inbox.");
requires(firebaseClient, /async function loadClanApplications[\s\S]*?where\("resetGeneration",\s*"==",\s*RESET_GENERATION\)[\s\S]*?where\("worldId",\s*"==",\s*ONLINE_WORLD_ID\)[\s\S]*?\.\.\.getRealmShardQueryConstraints\(where\)/, "The clan application fallback query does not use legacy-compatible shard access.");
...
requires(firebaseClient, /async function loadClanMembers[\s\S]*?where\("resetGeneration",\s*"==",\s*RESET_GENERATION\)[\s\S]*?where\("worldId",\s*"==",\s*ONLINE_WORLD_ID\)[\s\S]*?\.\.\.getRealmShardQueryConstraints\(where\)[\s\S]*?orderBy\("joinedAtMs"/, "Public clan rosters do not use a legacy-compatible shard member query.");
requires(firebaseClient, /function subscribeClanState[\s\S]*?collection\(client\.db,\s*"clans",\s*safeClanId,\s*"members"\)[\s\S]*?where\("resetGeneration",\s*"==",\s*RESET_GENERATION\)[\s\S]*?where\("worldId",\s*"==",\s*ONLINE_WORLD_ID\)[\s\S]*?\.\.\.getRealmShardQueryConstraints\(where\)/, "The live clan member listener does not use legacy-compatible shard access.");
...
requires(client, /function isHostileClanMarch[\s\S]*?mission\.returning[\s\S]*?mission\.reinforcementReturn[\s\S]*?mission\.campReturn[\s\S]*?mission\.relinquishTransfer[\s\S]*?return mission\.kind === "attack"\s*\|\| Boolean\(mission\.rallyAttack\)/, "Active clan attacks are not distinguished from allied movement, scouting, and return marches.");
...
requires(updateArmyTokenElementSource, /const clanAlly = isCurrentClanmateArmy\(attack\)[\s\S]*?clanAlly[\s\S]*?"clan-ally"[\s\S]*?clanAlly && isHostileClanMarch\(attack\) \? " clan-attack"[\s\S]*?\$\{ownerClass\}\$\{marchClass\}[\s\S]*?relationshipLabel = clanAlly \? "Clan ally "/, "Moving clanmate army markers do not distinguish allied attacks from movement while retaining an accessible label.");
...
requires(client, /function showPublicClanDetails[\s\S]*?data-public-clan-member-flag[\s\S]*?FlagRenderer\.render\([\s\S]*?member\.flag[\s\S]*?context:\s*"public-clan-roster"/, "Public clan profiles do not render member kingdom flags.");
...
requires(client, /function saveClanShieldEditor[\s\S]*?validateV2Write[\s\S]*?updateClanProfile\(\{\s*shield\s*\}\)[\s\S]*?heraldryRevision[\s\S]*?clanSnapshot\s*=\s*preserveNewerClanHeraldry\(\{[\s\S]*?savedShield/, "Clan Heraldry editor does not retain a strict, revisioned server-confirmed v2 shield.");
...
requires(styles, /\.army-route-ribbon\.clan-support-route\s*\{\s*fill:\s*rgba\(143,\s*226,\s*165,/, "Allied movement route ribbons do not use light green.");
requires(styles, /\.army-route-flow\.clan-support-route\s*\{\s*stroke:\s*rgba\(221,\s*255,\s*229,/, "Allied movement route flows lack their pale green highlight.");
requires(styles, /\.army-route-ribbon\.clan-hostile-route\s*\{\s*fill:\s*rgba\(67,\s*173,\s*98,/, "Allied attack route ribbons do not use medium green.");
...
requires(styles, /\.army-token\.clan-ally\s*\{[\s\S]*?color:\s*#102f1b[\s\S]*?border-color:\s*#ddffe5[\s\S]*?background:\s*linear-gradient\(#b3f2c2,\s*#8fe2a5\)/, "Allied movement markers do not use light green with readable dark text.");
requires(styles, /\.army-token\.clan-attack\s*\{[\s\S]*?border-color:\s*#8fe2a5[\s\S]*?background:\s*linear-gradient\(#68c982,\s*#43ad62\)/, "Allied attack markers do not use medium green.");
