/* Browser integration coverage for the approved Gatehouse, using synthetic gear and a local API stub. */
const fs = require('node:fs'), path = require('node:path'), assert = require('node:assert/strict');
const root = path.resolve(__dirname, '..'), out = path.join(root, 'release-artifacts/gatehouse-gear');
...
function add(slot, level, equipped = false, isNew = false) {
  const definition = G.DEFINITIONS.find(d => d.buildingId === 'gatehouse' && d.slot === slot);
  const instanceId = `gatehouse-qa-${++sequence}`;
  gear.instances[instanceId] = G.normalizeInstance({ instanceId, gearKey: definition.gearKey, level, isNew, acquiredAtMs: sequence });
  if (equipped) gear.equipped.gatehouse[slot] = instanceId;
  return instanceId;
}
  ...
  try {
    ...
    for (const [width, height] of [[1440,900],[1024,768],[844,390],[667,375],[568,320]]) {
      ...
      for (const [example, id] of [['ready',ready],['missing',missing],['max',max],['empty',''],['gold',ready]]) {
        await evaluate(`state.gold=${example === 'gold' ? 12000 : 128400};selectedCommonGearInstanceId=${JSON.stringify(id)};selectedCommonGearSlot=${JSON.stringify(id ? gear.instances[id].slot : 'gloves')};selectedCommonGearBagFilter='all';commonGearMergeConfirmOpen=false;renderCommonGearBuilding('gatehouse');`);
        await evaluate("Promise.all([...modal.querySelectorAll('img')].map(i=>i.decode()))"); await paint();
        assert(await evaluate(`(()=>{const image=modal.querySelector('[data-gatehouse-officer]'),bounds=image.getBoundingClientRect(),frame=image.parentElement.getBoundingClientRect();return image.complete&&image.naturalWidth===400&&image.naturalHeight===800&&image.getAnimations().length===0&&image.currentSrc.includes('gatehouse-defensive-commander-still-')&&bounds.x>=frame.x&&bounds.y>=frame.y&&bounds.right<=frame.right&&bounds.bottom<=frame.bottom;})()`), `Static character must decode and fit its frame at ${width}x${height}.`);
        ...
        if (example === 'max') assert(data.text.includes('Reduction to repair time added by new wall damage'));
        if (example === 'gold') assert(data.text.includes('Insufficient gold'));
      }
    }
    await evaluate(`state.gold=128400;selectedCommonGearInstanceId=${JSON.stringify(ready)};renderCommonGearBuilding('gatehouse')`); await paint();
    await click('[data-gear-merge]');
    assert(await evaluate("modal.querySelector('[data-gatehouse-officer]').getAttribute('src')===GATEHOUSE_OFFICER_ART"), 'The Commander must remain static during confirmation.');
    ...
    assert(await evaluate("modal.querySelector('[data-gatehouse-officer]').getAttribute('src')===GATEHOUSE_OFFICER_ART"), 'The Commander must remain static after confirmation.');
    ...
    await evaluate("state.gear=normalizeCommonGearState({});renderCommonGearBuilding('gatehouse')");
    ...
    await evaluate(`state.gear=normalizeCommonGearState(${JSON.stringify(gear)});selectedCommonGearInstanceId=${JSON.stringify(ready)};renderCommonGearBuilding('gatehouse');window.__gatehouseCalls=[];window.__gatehouseOriginalApi=getOnlineApi;getOnlineApi=()=>({unequipCommonGear:args=>new Promise((resolve,reject)=>{window.__gatehouseCalls.push({action:'unequip',args});window.__gatehouseReject=reject;}),equipCommonGear:async args=>{window.__gatehouseCalls.push({action:'equip',args});throw Error('Synthetic equip rejection');},upgradeCommonGear:args=>new Promise(resolve=>{window.__gatehouseCalls.push({action:'upgrade',args});window.__gatehouseResolve=resolve;})});`); await paint();
    ...
    await evaluate("modal.querySelector('[data-gear-equip]').click();window.__gatehouseReject(Error('Synthetic unequip rejection'))");
    await wait('!commonGearActionInFlight'); assert.equal(await evaluate('__gatehouseCalls.length'), 1);
    await evaluate(`selectedCommonGearInstanceId=${JSON.stringify(stored)};renderCommonGearBuilding('gatehouse')`); await paint();
    await click('[data-gear-equip]'); await wait('!commonGearActionInFlight');
    assert.equal(await evaluate('__gatehouseCalls[1].action'), 'equip');
    await evaluate(`selectedCommonGearInstanceId=${JSON.stringify(ready)};renderCommonGearBuilding('gatehouse')`); await paint();
    await click('[data-gear-merge]'); await click('[data-gear-merge-confirm]');
    assert(await evaluate('commonGearActionInFlight && __gatehouseCalls[2].args.requestId && __gatehouseCalls[2].args.instanceId === '+JSON.stringify(ready)));
    await evaluate(`(()=>{const next=normalizeCommonGearState(state.gear);const receipt=COMMON_GEAR.consumeUpgradeInputs(next,${JSON.stringify(ready)},'gatehouse-upgraded',Date.now());next.updatedAtMs=Date.now();window.__gatehouseResolve({gear:next,upgradedInstanceId:'gatehouse-upgraded',consumedInstanceIds:receipt.consumedInstanceIds});})()`);
    await wait('!commonGearActionInFlight');
    assert(await evaluate("selectedCommonGearInstanceId==='gatehouse-upgraded' && createCommonGearViewModel('gatehouse').selected.level===3 && Object.keys(state.gear.instances).length===15"));
    await evaluate('getOnlineApi=window.__gatehouseOriginalApi');
    for (const mode of ['reduced','off','full']) {
      await evaluate(`setAnimationModePreference('${mode}')`); await paint();
      assert(await evaluate("modal.querySelector('[data-gatehouse-officer]').getAttribute('src')===GATEHOUSE_OFFICER_ART && modal.querySelector('[data-gatehouse-officer]').getAnimations().length===0"));
    }
    await evaluate("selectedCommonGearInstanceId=Object.values(state.gear.instances).find(i=>i.slot==='weapon').instanceId;renderCommonGearBuilding('gatehouse')");
    assert(await evaluate("modal.querySelector('.tg-details').textContent.includes('Defending soldier strength in all owned cities')"));
    ...
    console.log('PASS: Gatehouse runtime, 25 desktop/landscape states, static character decoding/containment across motion settings, Shield and Masonry Seal scope, 44px controls, gray rarity surfaces, confirmation focus/Tab/Escape, selection/filter/scroll, empty inventory, pending/error action guards, upgrade response identity, Back/Close and other-officer isolation.');
  } finally {
