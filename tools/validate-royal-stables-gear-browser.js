/* Browser integration coverage for the approved Royal Stables, using synthetic gear and a local API stub. */
const fs = require('node:fs'), path = require('node:path'), assert = require('node:assert/strict');
const root = path.resolve(__dirname, '..'), out = path.join(root, 'release-artifacts/royal-stables-gear');
...
function add(slot, level, equipped = false, isNew = false) {
  const definition = G.DEFINITIONS.find(d => d.buildingId === 'royal-stables' && d.slot === slot);
  const instanceId = `royal-stables-qa-${++sequence}`;
  gear.instances[instanceId] = G.normalizeInstance({ instanceId, gearKey: definition.gearKey, level, isNew, acquiredAtMs: sequence });
  if (equipped) gear.equipped["royal-stables"][slot] = instanceId;
  return instanceId;
}
  ...
  try {
    ...
    for (const [width, height] of [[1440,900],[1024,768],[844,390],[667,375],[568,320]]) {
      ...
      for (const [example, id] of [['ready',ready],['missing',missing],['max',max],['empty',''],['gold',ready]]) {
        ...
        if (example === 'ready') {
          assert(data.text.includes('Owned-city transfer and reinforcement speed'));
        ...
        if (example === 'max') assert(data.text.includes('Scout speed'));
    ...
    assert(await evaluate("modal.querySelector('[data-royal-stables-officer]').getAttribute('src')===ROYAL_STABLES_OFFICER_ART"), 'The Cavalry Master must remain static after confirmation.');
    ...
    await evaluate("state.gear=normalizeCommonGearState({});renderCommonGearBuilding('royal-stables')");
    ...
    await evaluate(`state.gear=normalizeCommonGearState(${JSON.stringify(gear)});selectedCommonGearInstanceId=${JSON.stringify(ready)};renderCommonGearBuilding('royal-stables');window.__royalStablesCalls=[];window.__royalStablesOriginalApi=getOnlineApi;getOnlineApi=()=>({unequipCommonGear:args=>new Promise((resolve,reject)=>{window.__royalStablesCalls.push({action:'unequip',args});window.__royalStablesReject=reject;}),equipCommonGear:async args=>{window.__royalStablesCalls.push({action:'equip',args});throw Error('Synthetic equip rejection');},upgradeCommonGear:args=>new Promise(resolve=>{window.__royalStablesCalls.push({action:'upgrade',args});window.__royalStablesResolve=resolve;})});`); await paint();
    ...
    await evaluate("modal.querySelector('[data-gear-equip]').click();window.__royalStablesReject(Error('Synthetic unequip rejection'))");
    await wait('!commonGearActionInFlight'); assert.equal(await evaluate('__royalStablesCalls.length'), 1);
    await evaluate(`selectedCommonGearInstanceId=${JSON.stringify(stored)};renderCommonGearBuilding('royal-stables')`); await paint();
    await click('[data-gear-equip]'); await wait('!commonGearActionInFlight');
    assert.equal(await evaluate('__royalStablesCalls[1].action'), 'equip');
    await evaluate(`selectedCommonGearInstanceId=${JSON.stringify(ready)};renderCommonGearBuilding('royal-stables')`); await paint();
    await click('[data-gear-merge]'); await click('[data-gear-merge-confirm]');
    assert(await evaluate('commonGearActionInFlight && __royalStablesCalls[2].args.requestId && __royalStablesCalls[2].args.instanceId === '+JSON.stringify(ready)));
    await evaluate(`(()=>{const next=normalizeCommonGearState(state.gear);const receipt=COMMON_GEAR.consumeUpgradeInputs(next,${JSON.stringify(ready)},'royal-stables-upgraded',Date.now());next.updatedAtMs=Date.now();window.__royalStablesResolve({gear:next,upgradedInstanceId:'royal-stables-upgraded',consumedInstanceIds:receipt.consumedInstanceIds});})()`);
    await wait('!commonGearActionInFlight');
    assert(await evaluate("selectedCommonGearInstanceId==='royal-stables-upgraded' && createCommonGearViewModel('royal-stables').selected.level===3 && Object.keys(state.gear.instances).length===15"));
    await evaluate('getOnlineApi=window.__royalStablesOriginalApi');
    for (const mode of ['reduced','off','full']) {
      await evaluate(`setAnimationModePreference('${mode}')`); await paint();
      assert(await evaluate("modal.querySelector('[data-royal-stables-officer]').getAttribute('src')===ROYAL_STABLES_OFFICER_ART && modal.querySelector('[data-royal-stables-officer]').getAnimations().length===0"));
    }
    await evaluate("selectedCommonGearInstanceId=Object.values(state.gear.instances).find(i=>i.slot==='weapon').instanceId;renderCommonGearBuilding('royal-stables')");
    assert(await evaluate("modal.querySelector('.tg-details').textContent.includes('Attack and rally march speed')"));
    ...
    for (const building of ['treasury','barracks','gatehouse']) {
    ...
    await evaluate("renderCommonGearBuilding('royal-stables')"); await paint(); await click('#closeModalBtn');
    ...
    console.log('PASS: Royal Stables runtime, 25 desktop/landscape states, static character decoding/containment across motion settings, Lance and Wayfinder Pendant scope, 44px controls, gray rarity surfaces, confirmation focus/Tab/Escape, selection/filter/scroll, empty inventory, pending/error action guards, upgrade response identity, Back/Close and other-officer isolation.');
  } finally {
