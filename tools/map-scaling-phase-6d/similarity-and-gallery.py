"""Multi-metric visual similarity analysis and QA gallery generation for Phase 6D."""
...
def analyze_similarity(packages: list[dict], renders: dict[str, dict]) -> tuple[dict, list[dict], dict]:
    pairs = []
    ...
    def macro_pair_summary(predicate) -> dict:
        matching = [
            pair for pair in pairs
            if predicate(package_by_key[pair["left"]], package_by_key[pair["right"]])
        ]
        similarities = [pair["visualSimilarity"] for pair in matching]
        return {
            "pairCount": len(matching),
            "nearDuplicatePairCount": sum(value >= NEAR_DUPLICATE_THRESHOLD for value in similarities),
            "averageVisualSimilarity": round_number(np.mean(similarities)) if similarities else 0,
            "p95VisualSimilarity": round_number(np.quantile(similarities, 0.95)) if similarities else 0,
            "maximumVisualSimilarity": max(similarities, default=0),
        }

    corner_signatures = Counter()
    corner_sides = {
    ...
    for package in packages:
        for corner, (first, second) in corner_sides.items():
            corner_signatures[
                f"{package['theme']}|{corner}|{package['barrierDecisions'][first]}|{package['barrierDecisions'][second]}"
            ] += 1
    macro_similarity = {
        "sameFoundationAsset": macro_pair_summary(
            lambda left, right: left["foundation"]["assetId"] == right["foundation"]["assetId"]
        ),
        "sameFoundationPresentation": macro_pair_summary(
            lambda left, right: (
                left["foundation"]["assetId"] == right["foundation"]["assetId"]
                and left["foundation"]["transform"] == right["foundation"]["transform"]
            )
        ),
        "samePerimeterCombination": macro_pair_summary(
            lambda left, right: left["hashes"]["barrierSelectionHash"] == right["hashes"]["barrierSelectionHash"]
        ),
        "sameRoadGeometry": macro_pair_summary(
            lambda left, right: left["hashes"]["roadLayoutHash"] == right["hashes"]["roadLayoutHash"]
        ),
        "cornerRepetition": {
            "totalCornerPresentations": sum(corner_signatures.values()),
            "uniqueCornerSignatures": len(corner_signatures),
            "mostCommonFrequency": max(corner_signatures.values(), default=0),
            "mostCommonShare": round_number(max(corner_signatures.values(), default=0) / max(1, sum(corner_signatures.values()))),
        },
    }
    analysis = {
        "method": {
            "version": "phase6d-multi-metric-v1",
    ...
    return analysis, pairs[:25], macro_similarity
...
def analyze_neighbors(packages: list[dict], renders: dict[str, dict]) -> tuple[dict, dict, list[tuple[str, str]]]:
    ...
    for (x, y), record in sorted(by_coordinate.items()):
        for dx, dy, side, opposite in ((1, 0, "east", "west"), (0, 1, "south", "north")):
            neighbor = by_coordinate.get((x + dx, y + dy))
            if not neighbor:
                continue
            neighbor_pairs.append((record["key"], side, neighbor["key"], opposite))
            if record["theme"] != neighbor["theme"]:
                transition_pairs.append((record["key"], neighbor["key"]))
    package_by_key = {record["key"]: record for record in packages}
    same_theme_deltas = []
    cross_theme_deltas = []
    transition_counts = Counter()
    transition_deltas = defaultdict(list)
    transition_edge_deltas = defaultdict(list)
    same_theme_edge_deltas = []
    cross_theme_edge_deltas = []
    blend_applied_pairs = 0
    for left_key, left_side, right_key, right_side in neighbor_pairs:
        ...
        if left["theme"] == right["theme"]:
            ...
            same_theme_edge_deltas.append(color_distance(
                renders[left_key]["features"]["edgeMeanRgb"][left_side],
                renders[right_key]["features"]["edgeMeanRgb"][right_side],
            ))
        else:
            ...
            edge_delta = color_distance(
                renders[left_key]["features"]["edgeMeanRgb"][left_side],
                renders[right_key]["features"]["edgeMeanRgb"][right_side],
            )
            cross_theme_edge_deltas.append(edge_delta)
            transition_edge_deltas[pair_key].append(edge_delta)
            left_band = any(band["side"] == left_side and band["family"] == pair_key for band in left.get("transitionBands", []))
            right_band = any(band["side"] == right_side and band["family"] == pair_key for band in right.get("transitionBands", []))
            if left_band and right_band:
                blend_applied_pairs += 1
    ...
    transition_detail = {
        key: {
            ...
            "averageSharedEdgeBandDistance": round_number(np.mean(transition_edge_deltas[key])),
            "maximumSharedEdgeBandDistance": round_number(np.max(transition_edge_deltas[key])),
            "deterministicTransitionBandApplied": key in ("east-north", "north-west"),
        }
    ...
    transitions = {
        ...
        "sameThemeAverageSharedEdgeBandDistance": round_number(np.mean(same_theme_edge_deltas)) if same_theme_edge_deltas else 0,
        "crossThemeAverageSharedEdgeBandDistance": round_number(np.mean(cross_theme_edge_deltas)) if cross_theme_edge_deltas else 0,
        "transitionBandPairCount": blend_applied_pairs,
        "deterministicBlendProfileImplemented": True,
        "blendWidthPixels": 96,
        "maximumBlendStrength": 0.24,
        "transitionFamilies": transition_detail,
        "sameWorldPaletteRange": cross_average < 80.0,
        "recommendation": "Retain the four locked identities and the narrow deterministic North/East and North/West convergence bands; no whole-map blending is needed.",
    }
...
def city_overlay(root: Path, output: Path, package: dict) -> Image.Image:
...
def generate_gallery(
    ...
    random_25 = deterministic_selection(packages, 25, "phase6d-random-gallery-v1")
    save_grid([
        label_card(load_thumbnail(output, package), f"{package['key']} {package['theme']}")
        for package in random_25
    ], 5, gallery / "random-25-maps.png")
    save_grid([
        label_card(city_overlay(root, output, package), f"{package['key']} 40 current-city overlays")
        for package in random_25
    ...
    baseline_root = root / "benchmark-results/map/phase-6c"
    baseline_results = json.loads((baseline_root / "phase-6c-results.json").read_text(encoding="utf-8"))
    baseline_packages = json.loads((baseline_root / "packages-index.json").read_text(encoding="utf-8"))["records"]
    baseline_by_key = {package["key"]: package for package in baseline_packages}
    before_after_cards = []
    for baseline_pair, current_pair in zip(baseline_results["mostSimilarPairs"][:25], most_similar[:25]):
        panel = Image.new("RGB", (408, 180), "#171b1e")
        images = [
            load_thumbnail(baseline_root, baseline_by_key[baseline_pair["left"]], (100, 75)),
            load_thumbnail(baseline_root, baseline_by_key[baseline_pair["right"]], (100, 75)),
            load_thumbnail(output, by_key[current_pair["left"]], (100, 75)),
            load_thumbnail(output, by_key[current_pair["right"]], (100, 75)),
        ]
        for index, item in enumerate(images): panel.paste(item, (index * 102, 0))
        draw = ImageDraw.Draw(panel)
        draw.text((4, 82), f"6C {baseline_pair['visualSimilarity']:.4f} / 6D {current_pair['visualSimilarity']:.4f}", fill="#f4ead2", font=ImageFont.load_default())
        draw.text((4, 102), "left pair: before | right pair: after", fill="#cbbd9f", font=ImageFont.load_default())
        before_after_cards.append(panel)
    save_grid(before_after_cards, 5, gallery / "before-after-phase6c-vs-phase6d.png")
    ...
    transition_cards = []
    transition_cards_by_family: dict[str, list[Image.Image]] = defaultdict(list)
    ...
    for family in required_families:
        ...
        for left_key, right_key in selected_pairs:
            ...
            transition_cards_by_family[family].append(panel)
    save_grid(transition_cards, 4, gallery / "directional-transitions.png")
    save_grid(transition_cards_by_family["east-north"], 3, gallery / "transition-north-east.png")
    save_grid(transition_cards_by_family["north-west"], 3, gallery / "transition-north-west.png")
    foundation_cards = []
    edge_cards = []
    road_cards = []
    for theme in ("north", "east", "south", "west"):
        ...
        for package in selected:
            ...
            foundation_cards.append(label_card(full.resize((200, 150), Image.Resampling.LANCZOS), f"{package['foundation']['assetId']} {package['foundation']['transform']}"))
            edge = full.crop((0, 0, 320, 72)).resize((240, 72), Image.Resampling.LANCZOS)
            edge_cards.append(label_card(edge, f"{theme} {package['barrierDecisions']['north']}", 18))
            road = full.crop((110, 0, 210, 100)).resize((180, 180), Image.Resampling.LANCZOS)
            road_cards.append(label_card(road, f"{theme} {package['roadFamily']}", 18))
    ...
    gallery_files = [
        ...
        "before-after-phase6c-vs-phase6d.png",
        ...
        "transition-north-east.png",
        "transition-north-west.png",
...
<title>Crownlands Phase 6D repetition gallery</title>
...
</head><body><h1>Crownlands Phase 6D development-only repetition gallery</h1><p>Locked Phase 6A/6B art direction. No production maps or runtime objects are present.</p>{sections}</body></html>
"""
...
def main() -> None:
    ...
    near_duplicates, most_similar, macro_similarity = analyze_similarity(packages, renders)
    neighbor_cohesion, directional_transitions, transition_pairs = analyze_neighbors(packages, renders)
    gallery = generate_gallery(root, output, packages, most_similar, transition_pairs)
    analysis = {
        "schemaVersion": 1,
        "phase": "6D",
        "developmentOnly": True,
    ...
    print(
        f"Phase 6D similarity analysis: {near_duplicates['method']['sameThemePairsEvaluated']} pairs, "
