...
const buildId = "20260807-animation-system-v1";
for (const [label, source] of [["index", index], ["service worker", worker]]) {
  ...
  requireMatch(source, /firebaseClient\.js\?v=20260807-animation-system-v1/, `The ${label} does not refresh the Firebase client.`);
  requireMatch(source, /game\.js\?v=20260807-animation-system-v1/, `The ${label} does not refresh the login UI.`);
}
