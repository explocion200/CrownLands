...
requireMatch(client, /callServerFunction\("joinGameServer"[\s\S]*client\.activeSessionSnapshot = activeSession;[\s\S]*startActiveSessionWatcher\(uid\);/, "The replacement watcher must start only after server acceptance.");
requireMatch(client, /snapshot\.metadata\?\.fromCache[\s\S]*activeSession\.revision[\s\S]*signOutForSessionReplacement/, "Replacement must ignore cache and use the accepted server revision.");
