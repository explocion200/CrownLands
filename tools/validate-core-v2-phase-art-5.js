...
const APPROVED_BASE = "996e54c279c442592bb31dfea032097d80fc7d0a";
const EXPECTED_BRANCH = "codex/core-v2";
const MANIFEST_HASH = "701068cd92127df1790c2759bcd4e9ed6f088896d3ee3a8e767fd30667085c6f";
const OUTPUT = path.join(ROOT, "benchmark-results", "map", "core-v2-phase-art-5");
const DOCS = path.join(ROOT, "docs", "map-scaling-audit", "core-v2", "phase-art-5");
const SPEC = require("./core-v2-phase-a/spec");

const EXPECTED = Object.freeze({
  "east-deed-camp": { coordinate: [2, -1], capacity: 60, mapType: "DEED_CAMP", objective: ["deed", 742, 533] },
  swiftgate: { coordinate: [1, 0], capacity: 60, mapType: "STRONGHOLD", objective: ["march_speed", 724, 543] },
  "east-support": { coordinate: [2, 0], capacity: 70, mapType: "SUPPORT", objective: ["none", undefined, undefined] },
  "southeast-holding-tower": { coordinate: [1, 1], capacity: 55, mapType: "HOLDING_TOWER", objective: ["holding_tower", 736, 555] },
  "east-southeast-relic-camp": { coordinate: [2, 1], capacity: 55, mapType: "RELIC_CAMP", objective: ["relic", 746, 558] },
});
...
const REQUIRED_GALLERY = [
  "five-art5-clean-candidates.png", "five-art5-runtime-overlays.png", "road-treatment-board.png",
  "structure-integration-board.png", "east-climate-board.png", "east-south-transition-board.png",
  "neighbor-continuity-board.png", "swiftgate-road-structure-board.png", "east-deed-settlement-board.png",
  "east-relic-differentiation-board.png", "east-support-70-city-readability-board.png", "twenty-map-core-style-board.png",
];
...
function validateBatch() {
  const index = readJson(path.join(OUTPUT, "art5-index.json"));
  assert.equal(index.phase, "Core v2 Phase ART-5");
  ...
  assert.equal(index.finishedCoreMapCountAfterApproval, 20);
  assert.equal(index.representedCoreCityCapacityBeforeArt5, 885);
  assert.equal(index.art5CityCapacity, 300);
  assert.equal(index.representedCoreCityCapacityAfterApproval, 1185);
  assert.equal(index.remainingSouthBatchCapacity, 295);
  ...
  assert.equal(hashObject({ ...index, indexHash: undefined }), index.indexHash, "ART-5 index hash mismatch");
  ...
  for (const entry of index.entries) {
    ...
    assert(expected, `Unexpected ART-5 key ${entry.key}`);
    ...
    if (entry.key === "swiftgate") assert.deepEqual([entry.objective.x, entry.objective.y], [724, 543]);
    if (["DEED_CAMP", "RELIC_CAMP", "HOLDING_TOWER"].includes(entry.mapType)) assert(Math.hypot(entry.objective.x - 724, entry.objective.y - 543) <= 48);
    ...
    for (const city of cities) {
      assert(!batchCityIds.has(city.id), `Duplicate ART-5 city ID ${city.id}`);
    ...
    regionPlans.set(plan.coreRegion.regionId, plan);
  ...
  assert.equal(batchCityIds.size, 300);

  for (const plan of regionPlans.values()) for (const [side, connection] of Object.entries(plan.coreRegion.topology.connections)) {
    if (connection.state !== "OPEN" || !regionPlans.has(connection.regionId)) continue;
    const neighbor = regionPlans.get(connection.regionId);
    const reciprocal = neighbor.coreRegion.topology.connections[connection.reciprocalSide];
    assert.equal(reciprocal.state, "OPEN");
    assert.equal(reciprocal.regionId, plan.coreRegion.regionId);
  ...
  for (const file of roots.flatMap(collectCityFiles)) for (const city of readJson(file)) {
    assert(!allIds.has(city.id), `Duplicate city ID across 20 Core maps: ${city.id}`);
    allIds.add(city.id);
    allCities += 1;
  }
  assert.equal(allCities, 1185);
  return { maps: 5, cities: 300, finishedCoreMaps: 20, representedCoreCities: 1185, remainingSouthCities: 295, rows };
}
...
function validateVisualReview() {
  ...
  assert.equal(blocker.art5BrowserAttempted, false);
  assert.equal(blocker.art5ArtChangedByBrowserAttempt, false);
...
function validateProductionSafety() {
  ...
  childProcess.execFileSync("git", ["diff", "--quiet", APPROVED_BASE, "--", "benchmark-results/map/core-v2-phase-art-2-v2/candidates", "benchmark-results/map/core-v2-phase-art-3/candidates", "benchmark-results/map/core-v2-phase-art-4/candidates", "benchmark-results/map/core-v2-phase-art-4/geometry"], { cwd: ROOT });

  const allowed = [
    "benchmark-results/map/core-v2-phase-art-5/", "docs/map-scaling-audit/core-v2/phase-art-5/",
    "tools/core-v2-phase-art-5/", "tools/run-core-v2-phase-art-5.js", "tools/validate-core-v2-phase-art-5.js",
    "benchmark-results/map/core-v2-phase-art-4/art4-index.json", "docs/map-scaling-audit/core-v2/phase-art-4/README.md",
    "docs/map-scaling-audit/core-v2/phase-art-4/VALIDATION_RESULTS.md", "tools/core-v2-phase-art-4/fixture.js",
    "tools/run-core-v2-phase-art-4.js", "tools/validate-core-v2-phase-art-4.js",
  ];
  ...
  assert.deepEqual(forbidden, [], `Files outside ART-5 development scope: ${forbidden.join(", ")}`);
  ...
  if (fs.existsSync(dist)) {
    ...
    while (queue.length) {
      ...
      for (const entry of fs.readdirSync(current, { withFileTypes: true })) {
        ...
        else if (/\.(?:html|js|css|json|svg|txt)$/i.test(entry.name) && /core-v2-phase-art-5|east-southeast-relic-camp|core-v2-swiftgate-p1-p0/i.test(fs.readFileSync(file, "utf8"))) leakage += 1;
      }
    }
  }
...
console.log(JSON.stringify({ phase: "Core v2 Phase ART-5", result: "PASS_STATIC_VISUAL_REVIEW", approvedBase: APPROVED_BASE, branch: EXPECTED_BRANCH, batch, visual, production, committed: false, pushed: false, deployed: false, merged: false, productionActivated: false }, null, 2));
