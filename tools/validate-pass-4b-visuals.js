...
const styles = `${read("styles.css")}\n${read("interface-theme.css")}\n${read("ui-contrast-correction.css")}\n${read("player-flag-editor.css")}`;
const flagRenderer = read("functions/flagRenderer.js");
...
assert.match(flagRenderer, /element\.style\.setProperty\("--flag-primary", normalized\.primary\)/);
assert.match(game, /data-flag-color="\$\{option\.value\}"/);
assert.match(game, /PLAYER_FLAG_CONFIG\.normalizeFlag\(flag, identityKey\)/);
assert.match(flagRenderer, /setProperty\("--flag-symbol-color"/);
assert.match(flagRenderer, /setProperty\("--flag-symbol-outline"/);
...
assert.match(game, /api\.syncPlayerIdentity/);
