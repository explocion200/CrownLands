...
const { BATCH_COORDINATES, BATCH_VERSION, buildBatch } = require("./core-v2-phase-b1/batch");

const ROOT = path.resolve(__dirname, "..");
const OUTPUT_ROOT = path.join(ROOT, "benchmark-results", "map", "core-v2-phase-b1");
const COMPOSER = path.join(ROOT, "tools", "map-scaling-phase-6d", "compose_map.py");
const QA_RENDERER = path.join(ROOT, "tools", "core-v2-phase-b1", "render_qa.py");
...
function main() {
  ...
  assert.equal(prototypes.reduce((sum, prototype) => sum + prototype.cities.length, 0), 280);
  assert.deepEqual(
  ...
  for (const prototype of prototypes) {
    ...
    const output = path.join(OUTPUT_ROOT, "prototypes", prototype.profile.key);
  ...
  const batchIndex = {
    schemaVersion: 1,
    phase: "Core v2 Phase B1",
    batchVersion: BATCH_VERSION,
    developmentOnly: true,
    productionActivated: false,
    publicationAllowed: false,
    approvedBase: "8909aeee24aa99ecb58851c41eccac36815b4a54",
    exactBatchMapCount: prototypeIndex.length,
    exactBatchCityCapacity: prototypeIndex.reduce((sum, prototype) => sum + prototype.exactCityCapacity, 0),
  ...
  batchIndex.indexHash = hashObject({ ...batchIndex, indexHash: undefined });
  writeJson(path.join(OUTPUT_ROOT, "batch-index.json"), batchIndex);
  runPython(QA_RENDERER, ["--root", ROOT, "--output", OUTPUT_ROOT]);
  console.log(`Core v2 Phase B1 generated ${prototypeIndex.length} development-only prototypes / ${batchIndex.exactBatchCityCapacity} cities.`);
