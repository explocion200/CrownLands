...
(async()=>{
 ...
 try{
  ...
  const layout=()=>ev(`(()=>{const ids=['profileBtn','leaderboardBtn','clanHudBtn','dailyLoginRewardBtn','inventoryBtn','shopBtn','cityListBtn','islandSwitchBtn','chatToggleBtn','logBtn','fullscreenBtn'];return ids.map(id=>{const e=document.getElementById(id),r=e.getBoundingClientRect(),s=getComputedStyle(e);return{id,width:r.width,height:r.height,hit:e.contains(document.elementFromPoint(r.x+r.width/2,r.y+r.height/2)),fits:r.x>=0&&r.y>=0&&r.right<=innerWidth+1&&r.bottom<=innerHeight+1,background:s.backgroundImage,color:s.color,border:s.borderColor};});})()`);
