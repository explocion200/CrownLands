...
const { BATCH_COORDINATES, BATCH_VERSION, buildBatch } = require("./core-v2-phase-art-6/batch");

const ROOT = path.resolve(__dirname, "..");
const OUTPUT_ROOT = path.join(ROOT, "benchmark-results", "map", "core-v2-phase-art-6");
...
function runPython(script, args) {
  ...
  const result = childProcess.spawnSync(pythonExecutable, [script, ...args], { cwd: ROOT, encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] });
...
function main() {
  ...
  assert.equal(prototypes.reduce((sum, prototype) => sum + prototype.cities.length, 0), 295);
  assert.deepEqual(prototypes.map(prototype => prototype.coreRegion.coordinate), BATCH_COORDINATES.map(coordinate => ({ ...coordinate, worldLayer: 0 })));
  fs.mkdirSync(OUTPUT_ROOT, { recursive: true });
  const entries = [];
  for (const prototype of prototypes) {
    assert(prototype.receipt.validation.valid, `${prototype.coreRegion.name} failed: ${prototype.receipt.validation.errors.join("; ")}`);
  ...
  const index = {
    ...
    approvedBase: "47b2e81d11370554af57fb8098f31da24b345d4d",
  ...
  writeJson(path.join(OUTPUT_ROOT, "art6-index.json"), index);
  console.log(`Core v2 Phase ART-6 prepared ${entries.length} development-only geometry packages / ${index.exactBatchCityCapacity} cities.`);
