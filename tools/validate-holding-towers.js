...
const visualConfig = read("objective-visual-config.js");
const resetArchitecture = read("tools/core-v2-reset-2/architecture.js");
...
// Pending Core reset rehearsals generate four neutral Towers and a zeroed Treasury collection.
assert.match(resetArchitecture, /holdingTowers\.TOWERS\.map\(tower\s*=>\s*holdingTowers\.createNeutralTowerState/);
assert.match(resetArchitecture, /holdingTowers:\s*holdingTowerState[\s\S]*?clanTreasuries:\s*\[\]/);
assert.match(resetArchitecture, /tower\.wallLevel !== 1[\s\S]*?tower\.wallIntegrityBps !== 10_000[\s\S]*?tower\.neutralDefenders !== 10_000_000/);
assert.match(resetArchitecture, /clan-treasury-reset-state/);

// Integration/security invariants: server-only mutation, atomic money, Rally-only attack, reports, returns, and old-world gate.
