...
vm.runInContext([
  ...
  between(source, "function isCurrentWorldArmy(", "function createGlobalStatsSnapshot("),
...
for (const invalidScope of [{ worldId: "old-world" }, { resetGeneration: "old-season" }, { realmShardId: "other-shard" }]) {
  assert.equal(snapshot({ profile: { ...profile, ...invalidScope } }).totalTowerTroops, 0,
    "The current realm guards must survive the accounting repair.");
}
