...
- UI bounds: use `Toggle UI Bounds` or press `U` in Region Edit to show the approximate in-game UI footprint for each city, stronghold, crown citadel, and camp. These borders are editor-only and help you manually space placements so labels and marker UI do not overlap.
