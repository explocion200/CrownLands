...
assert.match(gameSource, /catch \(error\) \{\s*onlineOwnedCitiesCacheComplete = false;[\s\S]*?onlineOwnedCitiesRefreshError = onlineLastError/, "Denied and timed-out roster reads must retain an explicit incomplete retry state.");
...
assert.match(indexSource, /firebaseClient\.js\?v=20260830-city-list-reliability-r1[\s\S]*?instant-economy-actions\.js\?v=20260829-city-upgrade-queue-stability-r1[\s\S]*?game\.js\?v=20260830-heartbeat-lifecycle-recovery-r1/, "Changed city-list client assets must have current cache-busting release tokens.");
assert.match(workerSource, /firebaseClient\.js\?v=20260830-city-list-reliability-r1[\s\S]*?game\.js\?v=20260830-heartbeat-lifecycle-recovery-r1/, "The offline shell must precache the current City List client assets.");
