...
// Since the last budget revision (90b8f2e), approved Tower buildings/controls,
// identity protection and fullscreen launch added 49,907 normalized shell bytes.
...
for (const url of staticCacheUrls) {
  ...
  if (relativePath === "play/index.html") {
    // Built from index.html with ./ changed to ../; artifact validation checks
    // exact parity after stamping. Count it even before dist has been built.
    installPrecacheBytes += normalizedTextBytes("index.html") + 1;
    continue;
  }
...
for (const asset of manifest.assets) {
  ...
  if (asset.id === "gatehouse-defensive-commander-still") assert(payload.length <= 90 * 1024, "The static Commander must stay within its 90 KiB allowance.");
  if (asset.id === "royal-stables-cavalry-master-still") assert(payload.length <= 90 * 1024, "The static Cavalry Master and horse must stay within their 90 KiB allowance.");
