...
assert.match(actions, /async function refreshInstantEconomyAfterFailure[\s\S]*?resumeInstantEconomyActionsAfterSync[\s\S]*?showInventoryModal\(\)/, "Rejected item actions must wait for authoritative revalidation and rebuild the restored Bag immediately.");
...
assert(index.includes("instant-economy-actions.js?v=20260909-city-details-r1"), "instant-economy-actions.js has a stale queue-stability cache stamp.");
assert(worker.includes("/instant-economy-actions.js?v=20260909-city-details-r1"), "instant-economy-actions.js is missing from the refreshed offline shell.");
assert(index.includes("game.js?v=20260909-city-details-r1"), "game.js has a stale Layer 1 travel cache stamp.");
assert(worker.includes("/game.js?v=20260909-city-details-r1"), "game.js is missing from the refreshed offline shell.");
assert(worker.includes('CACHE_VERSION = "20260909-city-details-r1"'));
