...
assert.match(index, /crownlands-build" content="20260828-hero-level-gold-rebalance-r1"/);
for (const asset of ["styles.css", "instant-economy-actions.js"]) {
  assert(index.includes(`${asset}?v=20260828-hero-level-gold-rebalance-r1`), `${asset} has a stale page cache stamp.`);
  assert(worker.includes(`/${asset}?v=20260828-hero-level-gold-rebalance-r1`), `${asset} is missing from the refreshed offline shell.`);
}
...
assert(index.includes("game.js?v=20260828-hero-level-gold-rebalance-r1"), "game.js has a stale City List cache stamp.");
assert(worker.includes("/game.js?v=20260828-hero-level-gold-rebalance-r1"), "game.js is missing from the refreshed offline shell.");
assert(worker.includes('CACHE_VERSION = "20260828-hero-level-gold-rebalance-r1"'));
