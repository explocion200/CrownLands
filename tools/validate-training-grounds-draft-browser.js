...
const directory=path.resolve(__dirname,"../release-artifacts/training-grounds");fs.mkdirSync(directory,{recursive:true});
...
(async()=>{
 ...
 try{
  ...
  for(const[width,height]of[[1440,900],[844,390],[568,320]]){
   ...
   assert.equal(result.art,B.art("training",4));assert.equal(await evaluate('$("#currentBenefit").textContent'),"+4%");assert.equal(await evaluate('$("#nextBenefit").textContent'),"+5%");
   await capture(width+"-overview");
   await evaluate('$("#rallyExample").open=true;$("#rallyExample").scrollIntoView({block:"end"})');
   assert.equal(await evaluate('$("#exampleTotal").textContent'),"+34%");assert.equal(await evaluate('$("#exampleNext").textContent'),"+35%");
   assert.equal(await evaluate('$("#examplePower").textContent'),"13,400 power");
   for(const [base,total,power] of [[0,"+4%","10,400 power"],[50,"+54%","15,400 power"]]){
    await evaluate(`$("#exampleBase").value="${base}";$("#exampleBase").dispatchEvent(new Event("change",{bubbles:true}))`);
    assert.equal(await evaluate('$("#exampleTotal").textContent'),total);assert.equal(await evaluate('$("#examplePower").textContent'),power);
   }
   assert(await evaluate('$("#exampleGain").textContent.includes("400 attack power")'));assert(!(await measure()).overflow);assert((await measure()).footerVisible);
   await capture(width+"-rally-example");
   await evaluate('$("#rallyExample").open=false');
   ...
   assert.deepEqual(await evaluate('({balance:model.treasury,level:model.level,target:model.project.target,current:$("#currentBenefit").textContent})'),{balance:4260000000-B.cost(5),level:4,target:5,current:"+4%"});
   ...
   for(const sample of["paused","attack","damage","other","low","member","unavailable"]){
    ...
    assert.equal(await evaluate('$("#currentBenefit").textContent'),"+4%");
    if(sample==="paused"){
     ...
     assert(await evaluate('$("#overviewPanel").firstElementChild.classList.contains("paused")&&$("#overviewPanel").scrollTop===0'),"Pause reason must appear first and be visible after changing examples");await capture(width+"-paused");
    }
   }
   ...
   for(const level of[0,1,7,10]){
    ...
    assert.equal(await evaluate('$("#currentBenefit").textContent'),"+"+B.bonus("training",level)+"%");assert.equal(await evaluate('$("#buildingArt").getAttribute("src")'),B.art("training",level));
  ...
  await client.send("Page.navigate",{url:address.url+"/docs/visual-qa/training-grounds/index.html?viewport=landscape&level=4"});
  await wait('document.getElementById("preview")?.contentDocument?.documentElement?.dataset.trainingReady==="true"');
