...
function atlasFeatureFrame(kind, citadel = false, bonusIcon = "", hasClanFlag = false) {
 ...
 if (kind === "tower") {
  ...
  crest = hasClanFlag ? "" : `<g class="neutral-tower-crest"><path d="M116 1h30v17l-15 12-15-12Z" fill="#354e36" stroke="#c4cc9b" stroke-width="1.5"/>
  ...
  crest = `<circle cx="131" cy="13" r="12" fill="#dab366" stroke="#755323" stroke-width="1.5"/>
  ...
  crest = citadel ? `<g class="citadel-crest">
 ...
 // Separate SVG stacking contexts keep each crest above every border overlay.
 return `<svg class="feature-frame frame-${kind}" viewBox="0 0 262 203" aria-hidden="true" focusable="false">${art}</svg>${crest ? `<svg class="feature-frame feature-crest crest-${kind}" viewBox="0 0 262 203" aria-hidden="true" focusable="false">${crest}</svg>` : ""}`;
}
